...
pub struct Desktop {
    ...
    pub __sliver_tip_x: f32,
    pub __sliver_tip_y: f32,
    pub win_menu: String,
...
"); };
    ...
    fn state_snapshot(&self) -> std::collections::HashMap<String, auto_lang::ui::auto_val::Value> {
        ...
        m.insert("__sliver_tip_x".to_string(), auto_lang::ui::auto_val::Value::Float(self.__sliver_tip_x as f64));
        m.insert("__sliver_tip_y".to_string(), auto_lang::ui::auto_val::Value::Float(self.__sliver_tip_y as f64));
...
"); };
            ...
            DesktopSurfaceMsg::ToggleDashboardBlank => {
                self.blank_menu = "".to_string();
                self.on(DesktopSurfaceMsg::SendCmd("dashboard_toggle".to_string()));
            }
...
");
    ...
    fn view(&self) -> View<Self::Msg> {
        if self.visible == "1" { View::col().style("w-full h-full").child(View::MouseArea { content: Box::new(View::col().style("w-full h-full p-3 bg-card/80 border rounded-xl overflow-hidden").child(View::col().style("w-full h-full items-center justify-center").child(if self.nrows == 0 { View::text_styled("No widgets".to_string(), "text-muted-foreground text-sm") } else { View::Empty }).build()).child(View::col().style("absolute top-1.5 right-2 items-center gap-1").child(if self.active_tab == "main" { View::button("系统").style("bg-muted border border-border text-foreground font-medium rounded-md hover:bg-muted/70 h-10 px-4 h-6 px-2 text-xs rounded-full bg-card/80 border text-muted-foreground hover:bg-accent").on_click(|_| DashboardPanelMsg::SelectTab("system".to_string())).build() } else { View::button("小组件").style("bg-muted border border-border text-foreground font-medium rounded-md hover:bg-muted/70 h-10 px-4 h-6 px-2 text-xs rounded-full bg-card/80 border text-primary hover:bg-accent").on_click(|_| DashboardPanelMsg::SelectTab("main".to_string())).build() }).child(View::MouseArea { content: Box::new(if self.show_close == "1" { View::text("×".to_string()) } else { View::Empty }), on_enter: None, on_exit: None, on_double_click: None, on_click: Some(DashboardPanelMsg::Close), on_context_menu: None, on_release: None, on_move: None, logical_extent: None, style: auto_lang::ui::style::Style::parse("h-6 w-8 items-center justify-center rounded-lg hover:bg-accent").ok() }).build()).build()), on_enter: Some(DashboardPanelMsg::ShowClose), on_exit: Some(DashboardPanelMsg::HideClose), on_double_click: None, on_click: None, on_context_menu: None, on_release: None, on_move: None, logical_extent: None, style: None }).build() } else { View::Empty }
    }
...
impl ShellStateAccess for Desktop {
    fn shell_write(&mut self, key: &str, value: auto_lang::ui::auto_val::Value) -> bool {
        match key {
            ...
            "__sliver_tip_x" => { if let Ok(v) = value.deserialize_into::<f32>() { self.__sliver_tip_x = v; } true }
            "__sliver_tip_y" => { if let Ok(v) = value.deserialize_into::<f32>() { self.__sliver_tip_y = v; } true }
