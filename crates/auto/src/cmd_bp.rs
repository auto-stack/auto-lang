...
use crate::{BpAction, OutputFormat};

/// Entry point dispatched from main. `format` is the global `--format` flag
/// (it also carries diagnostics semantics on other subcommands — `bp list`
/// only consumes `At`, everything else stays text).
pub fn run(action: BpAction, format: Option<OutputFormat>) -> Result<()> {
    match action {
        BpAction::List => list(format),
...
fn list(format: Option<OutputFormat>) -> Result<()> {
    match format {
        Some(OutputFormat::At) => {
            println!("{}", emit_catalog_at(&registry()));
            Ok(())
        }
        _ => list_text(),
    }
...
/// Escape a string into an `.at` double-quoted literal body (lexer escape set:
/// `\\ \" \n \r \t`; the lexer passes any other escape through verbatim, so
/// only emit the known six).
fn escape_at_string(s: &str) -> String {
...
/// Read a file and normalize line endings to LF so the emitted table is
/// byte-stable across Windows/Linux checkouts (CI drift-check compares the
/// committed artifact against a fresh run — EOL noise must not count).
...
/// PLAN-676 SD-01 (T-02): emit the whole catalog as an `.at` static table —
/// the bps-gallery discovery data source, replacing PLAN-640's
/// `import.meta.glob` zero-artifact ruling (VM arm cannot glob the disk;
/// dual-arm same-source requires a committed artifact, PLAN-625 registry.at
/// precedent). Data-only: consumers hand-write their filter helpers against
/// `all_bps()`. Deterministic output (kind/name byte order, sorted variants)
/// so CI drift-checks can diff against the committed file.
fn emit_catalog_at(reg: &BlueprintRegistry) -> String {
    let mut out = String::with_capacity(64 * 1024);
    out.push_str("// GENERATED by `auto bp list --format at` — do not edit.\n");
    out.push_str("// PLAN-676 SD-01: bps-gallery 目录发现数据源（取代 PLAN-640 import.meta.glob\n");
    ...
    for pkg in reg.iter() {
        ...
        let gotchas = pkg.gotchas.as_deref().map(read_lf).unwrap_or_default();
        // Sorted variant list for byte-stable output across runs.
        let mut variants: Vec<(&String, &std::path::PathBuf)> = pkg.references.iter().collect();
        variants.sort_by(|a, b| a.0.cmp(b.0));
        let refs = variants
            .iter()
            .map(|(variant, path)| {
                let source = read_lf(path);
                format!(
                    "{{ variant: \"{}\", source: \"{}\" }}",
                    escape_at_string(variant),
                    escape_at_string(&source)
                )
            })
            .collect::<Vec<_>>()
            .join(", ");
...
mod tests {
    ...
    // ---- PLAN-676 T-02: `auto bp list --format at` emitter golden ----

    #[test]
    fn bp_list_at_escapes_strings() {
        assert_eq!(escape_at_string("plain"), "plain");
        assert_eq!(escape_at_string("a\"b"), "a\\\"b");
        assert_eq!(escape_at_string("a\\b"), "a\\\\b");
        assert_eq!(escape_at_string("a\nb\r\nc\td"), "a\\nb\\r\\nc\\td");
    }

    /// Structural golden: deterministic ordering, one entry per line with all
    /// five keys, escaped string bodies (no raw newline inside an entry line),
    /// stable across two runs, and the T-01 package present.
    #[test]
    fn bp_list_at_golden_stable() {
        let reg = BlueprintRegistry::with_defaults();
        assert!(!reg.packages().is_empty(), "default catalog must scan");
        let out = emit_catalog_at(&reg);

        assert!(out.starts_with("// GENERATED by `auto bp list --format at`"));
        assert!(out.contains("pub fn all_bps() List"));
        // Every scanned package appears exactly once, in byte order.
        assert_eq!(out.matches("    { kind: ").count(), reg.packages().len());
        assert!(out.contains("layout/gallery-shell"), "T-01 package in catalog");

        // Byte-order stability: keys ascending.
        let keys: Vec<String> = reg.iter().map(|p| p.key()).collect();
        let mut sorted = keys.clone();
        sorted.sort();
        assert_eq!(keys, sorted);

        // Entry lines are single lines: each starts with the entry opener and
        // closes on the same line (escaped \n, never a raw one, mid-entry).
        for line in out.lines() {
            if line.starts_with("    { kind: ") {
                assert!(line.ends_with("},"), "entry line must self-close: {line:?}");
            }
        }

        // Deterministic: two emissions are byte-identical.
        let out2 = emit_catalog_at(&reg);
        assert_eq!(out, out2, "emitter must be deterministic");

        // References carry variant/source pairs (login has three variants).
        let login = reg.get("form", "login").unwrap();
        assert!(login.references.len() >= 3);
        assert!(out.contains("variant: \\\"minimal\\\"") || out.contains("{ variant: \"minimal\""));
    }
}
