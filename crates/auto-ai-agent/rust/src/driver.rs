...
impl PipelineDriver {
    ...
    pub fn with_gate_handler(&mut self, handler: fn(String) -> GateDecision) -> PipelineDriver {
        self.gate_handler = Some(handler.to_string());
        return self;
    }
    ...
    pub async fn drive_step(&self, task_msg: &str, step_id: &str, role_id: &str, last_handoff: Option<HandoffDocument>) -> Result<(), AgentError> {
    ...
    pub async fn resolve_gate_auto(&self, step_id: &str) -> Result<(), AgentError> {
    ...
    pub async fn handle_after_submit(&self, next: AdvanceResult) -> Result<(), AgentError> {
        match next {
            AdvanceResult::Completed => {
                self.on_event(PipelineEvent::Completed);
                return Ok(());
...
fn truncate_chars(s: &str, max: u32) -> String {
    ...
    return &s[0 as usize..max as usize];
}
