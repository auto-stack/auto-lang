...
impl crate::role_def::Role for ConfigRole {
    fn name(&self) -> String {
        return self.cfg.name.unwrap_or(self.base_name());
    }
    fn system_prompt(&self) -> String {
        let mut prompt: Option<String> = self.cfg.system_prompt.unwrap_or(self.base_prompt());
        let mut append: Option<String> = self.cfg.system_prompt_append;
        match append {
...
{}", prompt, extra),
    ...
    fn model_tier(&self) -> ModelTier {
        return self.cfg.model_tier.unwrap_or(self.base_tier());
    }
    fn model(&self) -> String {
        return self.cfg.model.unwrap_or(self.base_model_str());
    }
    fn temperature(&self) -> f64 {
        return self.cfg.temperature.unwrap_or(self.base_temp());
    }
    fn max_turns(&self) -> u32 {
        return self.cfg.max_turns.unwrap_or(self.base_turns());
    }
    fn allowed_tools(&self) -> Vec<String> {
        return self.cfg.tools.clone().unwrap_or_default();
    }
    fn memory_limit(&self) -> Option<u32> {
        return self.cfg.memory_limit.unwrap_or(self.base_mem());
    }
...
impl ConfigRole {
    pub fn base_name(&self) -> String {
        let mut base: Option<Box<dyn Role>> = self.base.clone();
        match base {
