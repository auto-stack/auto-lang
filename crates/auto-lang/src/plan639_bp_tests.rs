...
{}",
    ...
    // 绑定工件自身 use 导入 bp 参考实现（零副本账面：工件只持 use，不持源）。
    let bind_at = app_at
        .parent()
        .and_then(|d| d.join("bps/login_bind.at").canonicalize().ok())
        .expect("bind artifact exists (generated by `auto bp add --bind`)");
    let bind_result = crate::ui_gen::generate_component_from_file(
        &bind_at,
        crate::ui_gen::ComponentGenOptions::default(),
    )
