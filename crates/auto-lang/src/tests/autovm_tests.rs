...
fn test_type_literal_str() {
    // Note: String literal .type doesn't make semantic sense - .type is for expressions
    // String is already a value, not an expression
...
#[test]
fn test_type_object_field_int() {
    // Note: Chained dot access like { x: 1 }.x.type is not yet supported by parser
    // This is a parser limitation that affects all chained field access
    // Skipping this test for now
}

#[test]
fn test_type_object_field_str() {
    // Note: Chained dot access like { name: "key" }.name.type is not yet supported by parser
    // This is a parser limitation that affects all chained field access
    // Skipping this test for now
}
