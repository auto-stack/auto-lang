...
impl WidgetRegistry {
    ...
    /// BackendMapping。schema 缺失(加载失败)时保持现状不破坏。
    fn apply_schema_vue_mappings(&mut self) {
        let Some(schema) = crate::aura::default_schema_cached() else {
            return;
        };
        let fold = |s: &str| {
            s.chars()
        ...
        for (_, spec) in self.widgets.iter_mut() {
            // 三级折叠解析(spec 名/别名 → schema canonical;别名组吸收后
            // canonical 折叠键可能不同,如 autodown 组收编了 autodown_editor)
