...
pub type ShimFunc = Arc<dyn Fn(&mut BigVM) -> Result<(), VMError> + Send + Sync>;
...
impl NativeInterface {
    ...
    where
        F: Fn(&mut BigVM) -> Result<(), VMError> + Send + Sync + 'static,
    {
...
pub fn shim_print_i32(vm: &mut BigVM) -> Result<(), VMError> {
    // Expect arg on TOS.
    // Callee cleanup: logic assumes we pop the arg.
    let val = vm.ram.pop_i32();
    println!("{}", val);
...
pub fn shim_print_f32(vm: &mut BigVM) -> Result<(), VMError> {
