...
fn main() -> Result<(), AutoError> {
    ...
    println!(
        "{}",
        " ATTENTION: am.exe (AutoMan CLI) has been merged into auto.exe"
            .bright_red()
            .bold()
    );
    println!(
        "{}",
        " Please use the new unified 'auto' command for all tasks.".bright_white()
    );
    println!(
        "{}",
        "-------------------------------------------------------------".bright_red()
    ...
    println!();
    println!(
        "Try {} for a full list of commands.",
        "auto --help".bright_green()
    );
