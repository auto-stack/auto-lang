            ...
            TokenTree::Group(group) => {
                match group.delimiter() {
                    ...
                    Delimiter::Bracket => {
                        ...
                        // DEBUG: 立即返回，避免后续修改
                        let string_literal =
                            syn::LitStr::new(&result, proc_macro2::Span::call_site());
                        return quote! { #string_literal };
                    }
                    _ => {
                        // 其他分隔符 - 递归处理
                        result.push_str(&tokens_to_string_helper(
...
fn tokens_to_string_helper(tokens: Vec<TokenTree>, brace_depth: usize) -> String {
    ...
    // AutoLang 关键字列表
    let keywords = [
        "let", "mut", "fn", "return", "if", "else", "for", "while", "loop", "break", "use", "type",
    ];
    ...
    for (i, token) in tokens.iter().enumerate() {
        ...
        match token {
            ...
            TokenTree::Group(group) => {
                ...
                let inner = tokens_to_string_helper(
                    group.stream().into_iter().collect::<Vec<_>>(),
                    brace_depth
                        + (if group.delimiter() == Delimiter::Brace {
                            1
            ...
            match &tokens[i] {
                ...
                TokenTree::Literal(lit) => {
                    ...
                    if lit_str.starts_with('"') {
                        ...
                        let string_literal =
                            syn::LitStr::new(unquoted, proc_macro2::Span::call_site());
                        quote! { auto_val::Value::Str(#string_literal.into()) }
                    } else if lit_str.contains('.') {
                        // 浮点数
                        i += 1;
                        let float_literal =
                            syn::LitFloat::new(&lit_str, proc_macro2::Span::call_site());
                        quote! { auto_val::Value::Double(#float_literal as f64) }
                    } else {
                        // 整数
                        i += 1;
                        let int_literal =
                            syn::LitInt::new(&lit_str, proc_macro2::Span::call_site());
                ...
                TokenTree::Group(_) => {
                    ...
                    i += 1;
                    let string_literal =
                        syn::LitStr::new(&value_str, proc_macro2::Span::call_site());
                    quote! { {
                        use auto_lang::atom::AtomReader;
                        let mut reader = AtomReader::new();
...
pub fn rust_fn(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    for arg in func.sig.inputs.iter().rev() {
        if let syn::FnArg::Typed(pat_type) = arg {
            if let syn::Pat::Ident(pat_ident) = &*pat_type.pat {
                ...
                pop_args.push(quote! {
                    let #arg_name: #arg_type = crate::vm::ffi::convert::VMConvertible::pop_from_stack(task, vm)
    ...
    let expanded = quote! {
        ...
        pub fn #shim_name(task: &mut crate::vm::task::AutoTask, vm: &crate::vm::engine::AutoVM) -> Result<(), crate::vm::engine::VMError> {
            ...
            crate::vm::ffi::convert::VMConvertible::push_to_stack(&result, task, vm)
                .map_err(|e| crate::vm::engine::VMError::RuntimeError(e.to_string()))?;
