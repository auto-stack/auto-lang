...
new_spec_components:
  - "docs/specs/auto-lang/ui/overview.md：新增「双端一致门禁」与两条语言层已知坑——(a) 容器 padding 覆盖次序（单侧 > 轴 > 统一 > legacy）与 margin 整族（m-*/mx-*/my-*）折算；(b) icon 尺寸权威 = `.at` 的 `size:`（生成器发 `:size`，缺省类 w-5 h-5 让位），以及「lucide 表项是完整 SVG 文档、消费方必须取内层 markup 重包」的实现约束"
