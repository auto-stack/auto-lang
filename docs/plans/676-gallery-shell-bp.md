...
supersedes_spec_components: []   # 拟: [blueprint/project#gallery-discovery-PLAN-640]
new_spec_components: []          # 拟: [blueprint/layout-gallery-shell]
...
- [ ] **T-01** 新建 `blueprints/layout/gallery-shell/{spec.md, reference/default.at, gotchas.md}`：按 §5.1 落契约与骨架实现；popover vs Overlay 通道在此任务实勘裁定（ui-gallery popover 双臂现状 → 定）；registry 测试名单更新。依赖：无。→ AC-01。验证：`cargo check -p auto-lang && cargo t bp`（registry 系滤串）。
- [ ] **T-02** cmd_bp.rs List 增 `--format at` 发射器 + golden 测试；输出形态（const 表 vs widget 字段）现场勘定。依赖：T-01。→ AC-05（发射器半边）。验证：`cargo t bp_list`（或滤串）+ 手跑 `auto bp list --format at`。
- [ ] **T-03** bps-gallery 项目化：pac.at（dep bps）+ registry.at 生成产物落地 + app.at 壳消费 GalleryShell（slot 填充）+ `auto build` 双臂通过。依赖：T-01/T-02。→ AC-03（壳半边）。验证：`auto build`。
- [ ] **T-04** 内容页迁移：spec 正文/变体切换/参考源码/gotchas 三区入 app.at（数据源=registry.at），kindOrder 偏好序迁入；信息架构与现版对齐走查。依赖：T-03。→ AC-03/AC-07（功能面）。验证：双臂走查。
- [ ] **T-05** 双臂验证：bp-gate `gallery_shell` 单元（units.mjs + 基线）+ bps-gallery autoui-verifier 双端证据。依赖：T-04。→ AC-02/AC-04。验证：`pnpm gate` + verifier 产出。
- [ ] **T-06** CI 适配：build-bps-gallery.yml 改 `auto build` 通道 + registry 漂移校验步；README 更新（发现机制/跑法/退役说明）。依赖：T-02/T-03。→ AC-05/AC-06。验证：workflow 绿。
- [ ] **T-07** 手写壳退役：删 src/App.vue、src/bps.ts、src/router.ts、src/pages/、Prism 依赖与 vite 手写工程面；`grep -r "import.meta.glob" examples/bps-gallery` 零命中。依赖：T-05 全绿后。→ AC-07。验证：grep + `auto build` 复跑。
...
1. **D2 触发通道**（T-01 内实勘，阻塞面小）：设置弹层走 popover（ui-gallery
   现状）还是 absolute+Overlay（PLAN-536 T6 VM 先例）？默认主张：popover，
   若 VM 臂 popover 词汇面有缺口则 fallback Overlay 并在 gotchas 登记。
2. **D5 消费通道细节**（T-03 内勘定）：pac.at `dep bps` 声明后 junction 物化
   时机由 `auto build` 托管（bp-gate R-B 同款）；worktree 纪律=合并前摘净。
3. **registry.at 输出形态**（T-02 内裁定）：const 字符串表 vs widget 静态字段，
   以解析器支持面+双臂一致性判据现场定，不另请裁决。
4. **供用户确认的三项默认裁定**：①导航=selected_id 状态切换（非 routes）；
   ②BP 名=`layout/gallery-shell` 单变体 default；③PLAN-640 import.meta.glob
   零产物裁定由 SD-01 显式取代。
