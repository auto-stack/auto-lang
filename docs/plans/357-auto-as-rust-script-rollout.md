# Auto as Rust's Script Layer — Rollout Implementation Plan
...
**与现有 Plan 的关系**：本计划复用 Plan 355（`parity/` 框架与 8 库骨架）的产出。C2 在 Plan 355 基础上加 CI 与清理；D 在 Plan 355 的 `libs/` 下扩展新用例。如某子项目需更深专项设计，可单独立项并以本计划为依据。
...
│   ├── libs/                         # D1/D2/D3: 扩展用例
│   │   ├── serde_json/  regex/        # Plan 355 已有，D1 补 L1 验证
│   │   ├── cli_app/  trait_advanced/  # D2 新增
│   │   ├── generators/                # D2 新增
│   │   └── http_client_sync/          # D3 新增（tokio 见 Plan 355）
│   └── docs/
...
- 真正的三向对比在 `parity/`（Plan 355），不在此文件。
...
git commit -m "fix(tests): make conformance_tests.rs header honest about scope (Plan 357 C1)
...
```
（已于 Plan 357 C1 修复：conformance_tests.rs 注释已诚实降级为"AutoVM 输出回归测试"。）
```
...
git commit -m "docs(strategy): note C1 conformance header fix completed (Plan 357)"
```
...
git commit -m "chore(parity): remove DBG355 debug residue from runner/main (Plan 357 C2)
...
git commit -m "test(parity): confirm p1/p2 green, record divergences (Plan 357 C2)"
```
...
git commit -m "ci: add parity p1/p2 gate on PRs touching a2r/auto-lang/parity (Plan 357 C2)"
```
...
Expected: 确认是否存在及现有职责（Plan 355 可能有占位）。
...
L3 列表硬编码为 p3/p4 库名（来自 Plan 355 阶段表）。
...
git commit -m "feat(parity): add 'report' subcommand generating HTML dashboard (Plan 357 C3)
...
```bash
git add .github/workflows/parity-ci.yml
git commit -m "ci(parity): auto-generate and upload dashboard artifact (Plan 357 C3)"
```
...
**背景事实：**
- `parity/libs/serde_json/` 与 `parity/libs/regex/` 已存在（Plan 355），D1 聚焦"补到 L1 全绿 + 做成可展示用例"。
...
git commit -m "test(parity): serde_json reaches L1 three-way green (Plan 357 D1)"
```
...
git commit -m "test(parity): regex reaches L1 three-way green (Plan 357 D1)"
```
...
git commit -m "feat(parity): add cli_app use case (wc-style, no-dep pure Rust output) (Plan 357 D1)"
```
...
git commit -m "docs(examples): add user-facing Script-to-Ship demos for D1 use cases (Plan 357 D1)"
```
...
git commit -m "feat(parity): add trait_advanced use case (default methods, assoc types, bounded generics) (Plan 357 D2)
...
git commit -m "feat(parity): add generators use case (yield iterators, lazy chains) (Plan 357 D2)"
```
...
```bash
git add examples/script-to-ship-demos/
git commit -m "docs(examples): add trait-demo and generators-demo Script-to-Ship demos (Plan 357 D2)"
```
...
**目标：** 补 a2r 最大短板——async/await（当前仅 3 个 trivial 案例）。聚焦"同步 HTTP 客户端"（用 a2r-std 已有的 ureq/http 模块）与 tokio 子集（Plan 355 已有 `parity/libs/tokio/`）。
...
git commit -m "feat(parity): add http_client_sync use case (synchronous ureq-based) (Plan 357 D3)"
```
...
git commit -m "test(parity): tokio reaches sorted-TAP parity for spawn/join + channel (Plan 357 D3)"
```
...
```bash
git add examples/script-to-ship-demos/
git commit -m "docs(examples): add http-client-demo and async-demo Script-to-Ship demos (Plan 357 D3)"
```
...
git commit -m "feat(website): scaffold ScriptShipView component (Plan 357 B1)"
```
...
```bash
git add website/.vitepress/theme/components/ScriptShipView.vue
git commit -m "feat(website): ScriptShipView VM-run + a2r-transpile logic (Plan 357 B1)"
```
...
git commit -m "feat(website): ScriptShipView side-by-side compare mode (Plan 357 B1)"
```
...
git commit -m "feat(website): register ScriptShipView global component (Plan 357 B1)"
```
...
git commit -m "feat(website): support <Listing view='scriptship'> -> ScriptShipView (Plan 357 B2)"
```
...
git commit -m "docs(script-to-ship): 6-chapter interactive tour (EN) (Plan 357 B2)"
```
...
git commit -m "docs(website): wire Script-to-Ship tour into sidebar (Plan 357 B2)"
```
...
git commit -m "docs(script-to-ship): Chinese translation of 6-chapter tour (Plan 357 B2)"
```
...
git commit -m "docs(website): v1 hero demo + narrative (L2 claims only) (Plan 357 A1)"
```
...
git commit -m "docs(website): three-act narrative + comparison table + status declaration (Plan 357 A1)"
```
...
```bash
git add website/index.md
git commit -m "docs(website): upgrade hero to L1-verified use case (Plan 357 A2)"
```
...
```bash
git add website/index.md
git commit -m "docs(website): link status declaration to live parity dashboard (Plan 357 A2)"
```
...
git commit -m "docs(website): bilingual landing + tour cross-link (Plan 357 A2)"
```
