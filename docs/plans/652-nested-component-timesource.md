...
- [ ] **AC-01** child `.Tick` 进入候选：单测断言 child_decls 含 Tick 的组件出现在 timesources/candidates，`every_ms` 与 `interval`（或默认 1000）一致。
- [ ] **AC-02** 挂载过滤：未 mounted ⇒ 订阅查询不含该 Tick/Timer；mounted 后含；root/store 恒订策略符合 §5.2。
- [ ] **AC-03** when 门不回退：`timer_when_allows_subscription` 既有测绿；假 when 的 child Timer 仍不订阅。
- [ ] **AC-04** 派发：合成/实机 child Tick 执行后状态更新（单测 handler 置脏/字段变化 **或** gallery `w_local` 走时）。
- [ ] **AC-05** 文档/债：SD-01..03 内容在 plan 内成文（merge 阶段落 specs）；设计文档与 00-intro/autoui README 已链 PLAN-652。
- [ ] **AC-06** 实机 gallery：012-clock 选中走时；切 demo 后停 Tick；standalone 不回归。
- [ ] **AC-07** 门禁：`cargo check --features ui-iced` 通过；相关单测绿；650 相关测不红。
- [ ] **AC-08** 静止不劣化：无 Tick 挂载的 gallery 静止 rebuild 频率与 650 后基线同量级（允许测量噪声；禁止全量 child 常订导致数量级回升）。
...
- [ ] **T-01 勘察装载名与派发面**  
  ...
  - 产出：§8 注记或 §10 阻断项（若 name 冲突需 rename 策略）。  
  - 验证：代码路径说明 +（可选）装载日志/单测打印 decl 名。  
  - AC：AC-04 前置  

- [ ] **T-02 TimeSource 收集 API**  
  - 位置：`crates/auto-lang/src/ui/dynamic.rs`（+ 必要时 `handler_codegen.rs` helper）。  
  - 操作：child/root/store 的 `.Tick`+`timer` 写入候选；公开 `subscribable_timesources()` / 等价查询。  
  - 验证：`cargo check -p auto-lang --features ui-iced`；新单测 AC-01。  
  - AC：AC-01  

- [ ] **T-03 mounted_types 装配登记**  
  - 位置：`aura_view_builder.rs`（及 dirty 重建入口清空逻辑）。  
  - 操作：实例化 child widget 时 insert 名；root/store 策略落地。  
  - 验证：单测 AC-02（mount 集合变化 → subscribable 变化）。  
  - AC：AC-02  

- [ ] **T-04 renderer 订阅循环改造**  
  - 位置：`renderer.rs` subscription ~18666。  
  - 操作：Tick/timer 统一 mount+when 过滤；避免与旧 `tick_interval()` 双订阅。  
  - 验证：check + AC-02/03 相关测；650 测回归。  
  - AC：AC-02 AC-03 AC-07  

- [ ] **T-05 派发路径打通**  
  - 位置：`vm_bridge.rs` / update 侧 widget 路由。  
  - 操作：子 widget `Tick`/`timer` 事件进对应 handler。  
  - 验证：单测或最小 VM 语料 AC-04。  
  - AC：AC-04  

- [ ] **T-06 实机 gallery + standalone**  
  - 靶：`D:/autostack/auto-os/ui-gallery`，`AUTO_GALLERY_APPS=D:/autostack/auto-lang/examples/ui`。  
  - 操作：MCP 驱动选 demo / state / Tick 计数；standalone 012-clock 冒烟。  
  - 验证：AC-06 脚本或手工记录（JSON/日志进 `scratch/p652/` 或 plan evidence）。  
  - AC：AC-06 AC-08  

- [ ] **T-07 规范增量成文 + 门禁汇总**  
  - 操作：SD-01 文件草稿可放 worktree `docs/specs/...`（merge 再进 master）或 plan 内全文；**plan 内必须有 SD 表与验收映射**。  
  - 验证：`cargo check` + 滤测；AC-05 勾选证据。  
  - AC：AC-05 AC-07  

- [ ] **T-08 work 收执 handoff**  
  - 操作：frontmatter `current_step`；§9 work 记录；`status: execution_done`；指向 `/auto-plan:review`。  
  - AC：过程  
