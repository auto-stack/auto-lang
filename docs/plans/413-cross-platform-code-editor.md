...
> **状态**: 🚧 实施中 —— **Phase 0 ✅ 已通过闸门**(2026-08-19,Windows 11 双后端验证,见 `scratch/code-editor-spike/README.md` 验证矩阵:fill_raw wgpu+tiny-skia 双通、syntect 单实例 5.3.0、fork API 差异表产出、iced_test 无头管线 5/5)。当前推进:Phase 5 测试与验收 —— **Phase 4 ✅ 已完成**(registry CodeEditor spec(vue→CodeMirror 目标 + npm 包声明,ark/jet 占位);vue.rs map_tag 特判降级 Textarea + attr/event/force_native 对齐;widgets-gallery code-editor 页(VM 试金石,PrintWindow 验证:行号+高亮+工具栏全渲染)+ 路由/nav;examples/ui/041-code-editor 迷你 Playground(VM 模式实机渲染验证通过))。 —— **Phase 3 ✅ 已完成**(ui_gen/rust.rs code_editor 特殊块生成 §3.3 形态 builder 链;code_editor_sources 注册使 handler 侧用 code_editor_text(key) 读值;KNOWN_TAGS/tag_to_view_fn;scan_input_fields 并入;auto-man 模板 iced 放宽 0.14,code-editor feature 经 ui-iced 链自动透传;codegen 测试 1 个 + ui_gen 646/646) —— **Phase 2 ✅ 已完成**(View::CodeEditor 变体 + builder + map_msg;aura_view_builder 双 tag arm + convert_code_editor(含子标签 content/oninput/oncursor/oncontextmenu);renderer into_iced 通用 arm + render_dynamic_view VM arm(INPUT_TEXT payload);vnode/snapshot 降级;gpui 降级 arm;MCP find_first_input 纳入 code_editor)。**Phase 1 ✅ 已完成**(三层结构 `ui/code_editor/{core,draw.rs,theme.rs,iced/}` 落地,core/draw/theme 零 iced 引用(编译验证);feature `code-editor` 挂 `ui-iced`;core 层 6/6 无头测试(键入/选区/撤销/IME/配置 diff/value diff 不跳光标)+ highlight 3/3;行为:word-motion、三击、滚动条拖拽、拖选自动滚动、Ctrl+Backspace/Delete 词删除、undo 后陈旧选区守卫)。
