...
> **状态**: 🚧 实施中 —— **Phase 0 ✅ 已通过闸门**(2026-08-19,Windows 11 双后端验证,见 `scratch/code-editor-spike/README.md` 验证矩阵:fill_raw wgpu+tiny-skia 双通、syntect 单实例 5.3.0、fork API 差异表产出、iced_test 无头管线 5/5)。当前推进:Phase 2 VM 模式接入 —— **Phase 1 ✅ 已完成**(三层结构 `ui/code_editor/{core,draw.rs,theme.rs,iced/}` 落地,core/draw/theme 零 iced 引用(编译验证);feature `code-editor` 挂 `ui-iced`;core 层 6/6 无头测试(键入/选区/撤销/IME/配置 diff/value diff 不跳光标)+ highlight 3/3;行为:word-motion、三击、滚动条拖拽、拖选自动滚动、Ctrl+Backspace/Delete 词删除、undo 后陈旧选区守卫)。
