...
> **已知残留**:① 冷启动高亮 ~4s(debug/onig,release 显著更低;可后续换 fancy 引擎);② vue 端为 Textarea 降级(CodeMirror shell 另立计划);③ VM 模式无 code_editor_text 原生函数(a2r 已支持,VM 值经 INPUT_TEXT 自动绑定);④ 路由切换自动 dispose 未接(有 code_editor_dispose API 待 app 层调用)。
