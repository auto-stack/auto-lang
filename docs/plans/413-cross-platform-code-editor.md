...
> **已知残留**:① 冷启动高亮:release 构建实测 1MB set 8ms / 冷渲染 595ms / 稳态 75µs(达标);debug 构建冷渲染 ~3s(onig 首次正则编译,仅影响开发期);② vue 端为 Textarea 降级(CodeMirror shell 另立计划);③ rust 模式 CLI 构建(`auto build` 的 rust 端口)在本机预存损坏(rust-counter 示例同样失败,与 413 无关)—— a2r 正确性经 e2e 编译测试覆盖;④ VM 模式 handler 表达式仍不能调用 payload 函数(文本经 INPUT_TEXT 自动绑定已覆盖主场景)。
