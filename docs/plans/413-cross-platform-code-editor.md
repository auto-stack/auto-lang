...
> **残留清零(2026-08-19 residuals pass,分支 plan-413-residuals)**:① **CLI rust 端口修复** —— build() 接通 build_rust_ui(与 run_rust_ui 对称)、共享 workspace 走位锚定、CARGO_TARGET_DIR 固定到仓库共享 target、workspace 模板补 ureq;041 rust 模式 `auto build && auto run` 实测端到端(生成 exe + 窗口渲染)。② **VM payload 原生函数** —— code_editor_text/cursor_line/cursor_col/selection_len/find/set_text 六个 natives(catalog+shims+codegen),.at handler 表达式两端(VM/a2r)一致可用;gallery 显示 Ln/Col。③ **vue CodeMirror shell 落地** —— 脚手架写 src/components/CodeEditor.vue(vue-codemirror + 6 语言包,write-if-missing + build/run 自愈)+ package.json 静态依赖(树摇);map_tag 翻回真组件(v-model/lang/wrap/search 全 prop 契约);vue 工程实机构建通过(产物含 cm-editor + 源码)。
> **剩余事项**:人工验收清单(IME 实机/150% DPI/Linux 复验/TESTING.md 交互)不变;`@codemirror` 深化(主题/搜索 UI/多光标)属后续增强。
