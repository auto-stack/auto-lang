...
| 640 | chart 族与 `data-table` 未入 WidgetRegistry——bp palette 回避裁定 | schema/aura.at 有 area-chart/bar-chart/line-chart/donut-chart 元素声明（aura.at:2422-2724）,但 Rust 侧注册已由 Plan 484 退役（registry.rs:2162-2164:裸名让位 official 包 Auto 组件,vue 发射臂死代码化）→ `palette_drift` 必漂移。PLAN-640 探针实证:probe 包 palette 塞全部候选 tag,漂移清单恰为 chart 四 tag。裁定:dashboard/overview palette 不含 chart,图表面走 extension_point `charts`（EDIT region,消费方在 app 层引 official 图表包）;`data-table`/`form-field` 无 aura 映射,palette 用 `table`/`form` 族名回避。后续若需 bp 内置 chart,须另立计划注册 chart 四 tag 进 `WidgetRegistry::with_defaults()`（触及 Plan 484 语义）。 |
