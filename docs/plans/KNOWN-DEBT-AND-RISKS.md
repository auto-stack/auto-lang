...
| 649 | 预存红（PLAN-649 T-05 兜底发现，非 649 引入）：`test_display_family_codegen_arm_fixture` icon size 断言失败 | `ui_gen::rust::tests::test_display_family_codegen_arm_fixture` 红——fixture `icon (name: "search", size: 14, ...)` 期望 `w-[14px] h-[14px]`，实际生成 `w-[20px] h-[20px]`（size prop 未进 icon 臂字面量提取，落 20px 默认档）。**归属实证（PLAN-649 2026-09-18 A/B）**：stash 649 全部改动后在干净基线 b13af592 同样红；`b13af592..master` 区间 rust.rs/fixture 零提交 → master 同带此红，且 docs/plans 此前零登记（未在案预存红）。引入窗口在 2026-09-06（tv 实测无红）～2026-09-17（b13af592）之间，非 PLAN-649 面（rust codegen 不读 WidgetRegistry，A/B 排除）。分诊方向：icon 臂 `props.get("size")` 的 `Expr::Int` 提取链（rust.rs:3227 一带）或上游 props 解析变更。 | `crates/auto-lang/src/ui_gen/rust.rs:3227`（icon 臂 size 提取）；PLAN-649 §9 T-05 记录 |
## 🟢 已知限制（设计决策，非 bug）
