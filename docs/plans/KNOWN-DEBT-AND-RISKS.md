...
- **P642-D3 [VM 实例 exit 127 无 panic 死亡类——P625 遗留，部分闭环]**：观测到的 127 有两个来源：(a) AutoUI MCP 端口被占时 bind FATAL 后进程静默退出（mcp_server.rs:540 FATAL 仅 return，不解释进程死亡）；(b) 画廊实例一的 029→screenshot 死亡。T-01 修复后 029 复验序列（press→screenshot）存活且出真图，实例二（并发 auto build + 窗口交互）死因未定。缓解候选：MCP bind 失败升级为显式错误面；死亡路径挂 guard/心跳留痕。引用：`/tmp/db026_vm.log`（bind FATAL 127 实证）、`/tmp/uigallery_vm.log`（实例一死前日志）。
