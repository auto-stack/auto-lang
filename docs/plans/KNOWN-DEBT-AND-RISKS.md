...
| p1(plan011④) | `__json_object` 浮点字段 Dot 读误码 | VM 轨 JSON 浮点字段单跳 Dot 读出错乱值：54.16(f64) 读出 -1073741824(0xC0000000)，`.floor()` 得 0；整数字段(as_i64→Int)与字符串正常。写入侧三级排除（os-config 探针实证 + 本仓核实：stdlib Number 分支产 `Value::Double`、对象字段经 `json_to_vm_value_inner` **直存** GenericInstanceData 无栈往返、GET_FIELD Double 臂 `push_f64` 正确）。位型指纹收敛为**读取链注入哨兵而非宽度错读**：54.16 裸 f64 位低 32=0xE147AE14、f32 位=0x4258A3D7，均非 0xC0000000——该值恰为 **-2.0f32 完整位型**（encode_f32 payload）或 int -2^30（=bool/null 哨兵 i32::MIN=0x80000000 算术右移 1 位），与已修同族 fb06cd8b2①（decode 兜底位读）/bb6608f75③（哨兵被 pop_i32 误解码）同源。注意裸 f64 不装箱（encode_f64=原始位，is_f64=!is_nanboxed），decode_i32 无 tag 守卫恒取低 32 位。排查面：GET_FIELD 结果 nv 的**消费方**（expr_type_hint 对 __json_object 未知字段若落 Int → 整型算术/插值按 i32 弹栈）、UI 状态回读桥、任何对哨兵做移位/折半的转换步。复现：auto-os-config worktree plan-011-dev（`system_info_json()` 浮点字段 + `auto run -r vm` + MCP `autoui_state`）；os-config 待澄清#5④ 已回写线索（117826b），T12 整数化绕开中（f85ab91），修复后撤绕法恢复原始数值字段。 | `stdlib.rs:2332-2340`/`2404-2411`（Number 分支）、`2363-2380`（字段直存）；`engine.rs:4975-5183`（GET_FIELD，Double 臂 :5144、__json_object 缺键 null :5173、receiver 兜底 decode_i32 :4988-5000）；`nano_value.rs:46/82/130`（nanbox 编解码）；auto-os-config `docs/plans/011-daemon-auto-back.md` 待澄清#5④ |
