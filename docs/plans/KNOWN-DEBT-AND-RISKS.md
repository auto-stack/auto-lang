...
| 671 | 基线预存红（非本批引入，复审 detached ebfeef30c 实证） | master 预存红三簇：①日常档 `cargo t` 20 红（musk_vm_track_p053/p054 ×7 + ui::layout ×13——full 配置 tf/tv 下不存在=日常档配置/环境面）；②`cargo tb` book_listing ×7（ch02_05/ch03_08/ch06_05/06/08/09/ch09_02，book 语料解析错）。668 绝对全绿口径后引入，需独立归因计划（本批全部对基线 diff 零新增）。 | PLAN-671 §9 复审记录；/tmp 比对留档（red_base/red_mine/red_tb_mine） |
