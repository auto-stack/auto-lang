...
| DIV-A2R-STRPARAM-1 | 一致性遗漏 | 🟡 | base64/url/serde_json 三库 a2r 编译级回归:局部 fn 的 `&str` 形参在调用点未自动借用(String 直传→E0308),三向降为 0-64%。已在前置点 2ac2298f 复现(早于 417-E3 批次,系 8-20~8-22 并行批次引入)。当前稳定全绿集 141 例/7 库 + consumer 3 库;网站声明已按现状口径收缩。专门批次待立 | `trans/rust.rs` fn_str_param_indices 调用点借用路径 | 2026-08-22 |
