...
| 572 | ✅已结算(Plan 574,2026-09-06): Windows 环境限制 aavm 进程内语料测试栈溢出族(12) | 裸 `cargo taa` 12 测试 STATUS_STACK_OVERFLOW——进程内双层解释(宿主 VM 跑 aavm.at+lib)递归**路径级**爆栈(001_smoke 单行 print 实证,与用例规模无关);基点即红,非 572 回归(两态失败集 13/13 逐名一致)。**〔2026-09-06 用户裁定→Plan 574 落地〕根因=avm+aavm/avm+aa2r 双重解释器路径非真实需求(2×2 对称性产物;真实自举=a2r 转译+编译+运行)。处置=12 测试 `#[cfg_attr(windows, ignore)]`(Windows 关闭,Linux/CI 保留全量);Windows 裸 taa 失败集 13→1(仅 charts_gallery 564-Q6 邻接);规约注记=aavm/project.md 验证矩阵节+AGENTS AAVM 档;对账表=scratch/p574/coverage-map.md。** | `scratch/p574/coverage-map.md`;572 待澄清②;Plan 574 |
