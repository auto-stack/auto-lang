...
- **P643-D1 [icon 词汇面缺口——schema builtin_widget ∉ vue 轨 WidgetRegistry（PLAN-643 merge 对账发现）]**：`icon` 在 schema/aura.at 为 `builtin_widget`（iced native，Plan 620/621 语义），但 ui_gen `WidgetRegistry::with_defaults` 无注册——bp palette 引用它即漂移（plan070 收编 navigation/filetree 时声明 `palette=["icon","text"]` 致 `palette_has_no_drift` 实红，被 070 实勘的 CARGO_MANIFEST_DIR 扫描根限制掩盖；PLAN-643 merge 时以 filetree palette 置空顺带恢复门禁）。与 chart 四 tag 同族（schema 权威 vs registry 滞后）但形态相反：chart 是 registry 有意退役让位组件包（643 已解），icon 是 registry 漏注册的 schema 内建。修向候选：①vue registry 补 icon 注册（对照 VM 原生实现）②palette 合法集扩为 registry ∪ schema native/builtin tier（扩大豁免面，需防 unclassified 杂音入列）。归属：vocabulary-face 整备（PLAN-648 门禁/codegen 缺陷簇整备或后续）。引用：`blueprints/navigation/filetree/spec.md` palette 注记；`palette_has_no_drift` master 复现（2026-09-18）。
