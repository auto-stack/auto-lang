...
| P026-D1 | low | 保真边界 | **~~图像真渲半句~~ ✅ 已核销（PLAN-028，协议 §1.9）**——`DrawOp::Image`（tag 6）入册：src 引用 + 宿主侧解析（词汇表/降级纪律/`thumbnail://` 虚拟引用），两投影臂真图升级，占位保真口径转宿主侧未解析兜底；**字形真渲半句维持 not-yet**（lucide:/svgdoc: 栅格化独立线，经未解析降级占位） | message.rs Image 变体；broker_surface.rs 图像解析面；client_runtime.rs layout_image / native_projector.rs Image 臂随注；desktop-protocol-v1.md §1.9 |
...
- **P028-D2 [lucide:/svgdoc: 字形/矢量真渲 not-yet（保真边界）]**：icon 经 a2r 降级形态 `image_styled("lucide:{name}")` 随图像通道入线，宿主解析不了字形词汇 → 未解析降级占位（行为与旧占位口径连续）；字形/矢量栅格化独立线（P026-D1 后半维持）。
