...
| 417-E4 调查 | parity 断裂 | trait_advanced 三方重跑仍阻于预存 a2r 缺口(此前被 async_stream 镜像错误掩盖,E1 修 runner 后暴露):值返回函数的 if 表达式尾臂发射带分号(`if c { a; } else { b; }` → unit,E0308);VM 侧直接运行通过(ok 1/ok 2)。修复点:trans/rust.rs 值位 if 表达式臂去分号 | `parity/libs/trait_advanced/build_a2r/default_methods_probe/src/main.rs:119` | 2026-08-22 |
