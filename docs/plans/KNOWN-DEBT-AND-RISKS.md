...
| P-RQ-PIX | low | 终态清理 | **pixels 臂退役门（2026-09-19 用户裁定意向入册）**——queue 臂完善后 pixels 臂（隐藏窗自渲+截图+RGBA shm 机件族）整体冗余可删。退役门四条件：①native auto 翻转（PLAN-032 在册）②widget 覆盖收口至像素原生族外全量（M7-c 后）③位图过线通道落地（028 not-yet 项——app 生成像素内容经图像算子流过协议，取代截图管线）④像素原生 widget 族（canvas/video/terminal/code_editor/imagesurface）去留裁定（过线 or 仅 inproc/独立窗）。删除面 = FrameMode::Pixels 档/pixels.rs（PixelsChild/run_independent_*）/NativePixelsHost/dual_mode 测试族/ **量化验收门（2026-09-19 用户定标）**：单 app `-q` 渲染正确 + app ≤10MB（031 实测 7-9MB ✅）+ rqhost ≤100MB（031 debug 实测 314MB ❌——release 复测 + wgpu/每窗表面/图像缓存归因优化为前置工作项）；达标后执行退役。 | desktop_protocol/pixels.rs；message.rs FrameMode；renderer.rs NativePixelsHost；dual_mode |
