...
| 525-3 | 生成器 yield 与 `??` NullCoalesce 延后 | 生成器:W0 盘点 lib 用量=0,按待澄清③裁定延后(宿主 Plan 321 在位);`??` 已入 Pratt 表但无语料面(未实现码 gen)。May 最小面(?T/Some/None/is 臂)已交付(g34)。 | `auto/lib/codegen.at`(?? 臂缺);宿主 `vm/codegen.rs` | 后续波次按需领取 |
| 525-4 | 宿主 May 裸值 return 发射不编译 | `fn f() ?int { return n*10 }`(无 Some 包裹)主 a2r 发 `return n * 10;` 于 Option<T> fn——rustc E0308。525 语料取显式 Some 构造规避(g34);宿主发射修复(裸值自动包 Some)待后续。 | `crates/auto-lang/src/trans/rust.rs` | 语料已规避;宿主修复后可补裸值语料 |
| 525-5 | ⑤腿塔顶程序 rc=1 快死(P517-1 族再现,W2-W5 折叠点) | 折叠②起矩阵两次+手动塔顶均 rc=1 快速返回无输出(P517-1 文档形态一致);lib 增长至 ~879KB 后贴线加剧。折叠①时点矩阵 46/46(10m4.6s)健康;各折叠点四路全绿+语料腿全绿为替代证据链(517 折叠①先例)。恢复后未复跑成功。 | `parity/crates/auto-parity/src/aavm.rs`(build_aa2r_bin);环境负载 | 空闲窗口复跑矩阵;若复现则独立分诊(非 525 改动引入——46 语料腿绿) |
## 🟢 已知限制（设计决策，非 bug）
