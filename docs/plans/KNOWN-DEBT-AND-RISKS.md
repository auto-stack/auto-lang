...
| P617-D9 | high | 未收口能力 | **AC-19 未达成**：T-19 已把 `video` 接到 `View::Video` 与 iced 渲染面，但**默认 `auto run -r vm` 看不到原生播放**——因为 `crates/auto` 没有 `mpv-widget` 的特性透传（默认走诚实降级面板），且应用需声明 tick 驱动帧。要收口须：① 补 `crates/auto` 的特性透传（T-20 的特性门控）；② 应用侧 `tick_interval_ms`；③ VM 实机截图验证 | 计划 §9.18；design doc §4.11；`crates/auto/Cargo.toml`（features 段） |
