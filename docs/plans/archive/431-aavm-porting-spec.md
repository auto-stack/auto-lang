...
- [x] A1 parser.rs 功能分区表（基于 429 调研的行号区间，校准到 v0.5 tag）：
...
- [x] A2 同法处理 `ast/`（32 文件：ui/task/route/on/tag/grid/cover 剔除）、
...
- [x] A3 opcode 处置表：194 条逐条标注 `移植`（核心执行路径）/`仅声明`（枚举占位，engine 不实现）/
  `剔除`（UI/并发/actor 专属）。**编号保持与 Rust 一致**（兼容性资产）。
- [x] A4 边界清单反向价值：标注"Rust 侧若按此边界拆分会更干净"的重构建议清单
  （喂给 432 执行期的债务记录，不提前重构）。

### Phase B：文件级映射表（1-2 天）

- [x] B1 映射表（Markdown，附于本文件）：
...
- [x] B2 每个 P0/P1 文件给出"移植顺序内 topo 依赖"（token→lexer→ast→parser→typeinfo→opcode→codegen→engine）。

### Phase C：divergence 规则（1-2 天）

- [x] C1 允许的改写模式清单（首版至少覆盖）：
...
- [x] D2 corpus 预算：依据 429 B2 性能报告限定单用例 VM 解释执行的时限，超限语料标记"仅 AAVM-Rust 跑"。

### Phase E：目录骨架与测试基建（2-3 天，代码）

- [x] E1 AAVM v2 目录：`auto/lib/` 重写为 v2（旧 13 文件移入 `auto/lib-legacy/` 归档 + README），
  `auto/pac.at` 更新；或新顶层目录——**在本计划定稿，二选一**（建议前者，保留 pac 构建回路）。
- [x] E2 VM 文件测试基建 v2：新用例目录 `test/vm/aavm2/`，runner 复用 99_bootstrap 的
  "lib 前置拼接"法（修掉双清单问题，429-A3 已做）；AUTO_LIB_FILES v2 清单单一事实源。
- [x] E3 a2r 编译对比 runner 骨架：`auto trans --merge` AAVM → cargo build → 运行 corpus →
  与预期输出 diff（复用 parity runner 的 TAP 对齐机制，仅搭骨架 + 一个冒烟用例）。
- [x] E4 `.at` 文件头 Snapshot 模板定稿（Rust ref / Baseline=v0.5 tag / Coverage / Missing，
  沿用旧机制、统一基线）。
...
- **Phase A**(docs/specs/aavm/design/porting-boundary.md + data/*.csv):
  ...
  - opcode 194 条处置表:移植 183 / 仅声明 6(SWAP/CONST_U8/SHL/SHR/JMP_L/PRINT
    ——SHL/SHR 正是 429-B3 的位移缺口)/ 剔除(并发) 5;
  - native_catalog 521 条:核心 455 / 剔除(UI/异步) 66;ast/ 33 文件剔 8、
    infer/ 10 文件剔 1(文件级);A4 拆分建议 5 条。
- **Phase B**(file-mapping.md):10 文件映射表 + topo 序(P0: token→error→lexer;
  P1: ast→parser;P2: typeinfo→opcode→codegen→engine→natives);
  AUTO_LIB_FILES_V2 登记规则。
- **Phase C**(divergence-rules.md):D1-D9 九类改写模式 + DIVERGE 记录格式 +
  纯 Rust 模式规范(含 lint 规则)+ E4 Snapshot 五字段模板。
- **Phase D**(corpus.md):四层语料(lexer/parser/执行/终局)+ 10s 预算 +
  slow.corpus 标记 + 登记约定。
- **Phase E**(代码):
  - E1:auto/lib(13 文件)→ auto/lib-legacy 归档(README 注明三类可回收资产),
    AUTO_LIB_FILES 指向归档路径(99_bootstrap 108 用例照常可跑),
    auto/lib 留给 v2(pac.at 构建回路保留);
  - E2:AUTO_LIB_FILES_V2 单一事实源(lib.rs)+ test_aavm2 runner +
    冒烟用例 001_smoke ✅;
  - E3:test_aavm2_compile 骨架(transpile→临时 cargo→build→run→diff)✅
    Hello 级冒烟 002_hello_compile 1.19s 通过;
  - E4:Snapshot 模板(divergence-rules.md §5)。
- **验证**:四条 Verification 全满足;全量 --lib 3129 绿(+feature 门下
  aavm2 两用例);已知 ui_console 并行偶发与本计划无关。

**给 432 的接口**:边界 CSV(初始假设,逐函数人工复核回写)、映射表 topo 序、
divergence 登记(随移植维护 divergences.md)、corpus 目录、v2 runner 与
AUTO_LIB_FILES_V2 登记回路。
