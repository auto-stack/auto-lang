...
| 6B-3.2 | Cargo.toml 自动生成 | ✅ **已完成** (Plan 167 Phase 6) | `transpile_rust_project()` 自动生成 Cargo.toml |
| 6B-3.3 | 动态值类型 `Value` | 无 | `type Value` → `serde_json::Value` 或自定义 enum |
| 6B-3.4 | 静态常量 `const` | ✅ test 157 | `const NAME str = "..."` → `const NAME: &str = "..."` |
| 6B-3.5 | 多文件模块系统 | ✅ **已完成** (Plan 167) | `transpile_rust_project()` + `discover_modules()` + `mod X;` 生成 |
...
- [x] a2r Phase 6B-3.2：Cargo.toml 自动生成 ✅ Plan 167 Phase 6
- [x] a2r Phase 6B-3.5：多文件模块系统 ✅ Plan 167 (tests 159-161)
...
### F.4 优先级 P1：`is` 语句完整 match arm ✅ 已完成

**当前状态**: ✅ `is_stmt` 已支持完整语句块作为 match arm body（test 132_is_multi_stmt）

### F.5 优先级 P1：`async fn` 声明 ✅ 已完成

**当前状态**: ✅ `async fn` 声明已支持（test 134_async_fn）

**影响的 AutoCode 模块**: Agent 循环、API 客户端

### F.6 优先级 P2：derive 宏与属性透传 ✅ 已完成

**当前状态**: ✅ `#[derive(...)]`, `#[serde(...)]` 等属性透传已支持（test 135_derive_attr）
...
### F.8 优先级 P2：外部 crate 引用 ✅ 已完成

**当前状态**: ✅ `use.rust reqwest::blocking::Client` → `use reqwest::blocking::Client;` 已支持（test 133_rust_use）
...
### F.11 优先级 P3：Cargo.toml 自动生成 ✅ 已完成（Plan 167 Phase 6）

**当前状态**: ✅ `transpile_rust_project()` 自动生成 Cargo.toml。

**已完成**:
- ✅ 从项目目录名生成 `[package]` 段
- ✅ 扫描 `.rust` use 语句生成 `[dependencies]` 段

### F.12 优先级 P3：多文件模块系统 ✅ 已完成（Plan 167）

**当前状态**: ✅ `transpile_rust_project()` 支持完整的多文件项目转译。

**已完成**:
- ✅ `discover_modules()` 递归发现所有模块（文件模块 + 目录模块）
- ✅ `use X` → `mod X;`（本地模块声明）
- ✅ `pub use X: item` → `pub use crate::X::item;`
- ✅ `use super.X` → `use super::X;`
- ✅ 目录模块 `mod.at` → `mod.rs` + `pub mod sibling;` 声明
- ✅ 点分模块名处理（如 `api.handlers`）
- ✅ `MultiSink` 多文件输出缓冲
- ✅ Tests: test_159_pub_use, test_160_wildcard_import, test_161_multi_file

### F.13 优先级 P3：静态常量 ✅ 已完成（test 157）

**当前状态**: ✅ `const` 声明已支持。

**已完成**: `const NAME str = "..."` → `const NAME: &str = "..."`

### F.14 a2r 已支持的关键特性（无需修改）
