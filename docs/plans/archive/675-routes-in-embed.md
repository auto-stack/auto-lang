...
- **T-08**(rev2)back_proxy lazy 会话按需装载:`BackProxyConfig.lazy_sessions` 档——启动零预 spawning,spec 进目录表;`ensure_session` 在会话表锁内完成「查目录→spawn→插表」(并发首击串行化防重),请求在 mpsc 排队至装载就绪(装载失败=既有 degraded 503);`has_session` 语义=在册或已载。画廊消费点(rust_ui start_gallery_back_proxy)置 lazy=true。动机=走查实证的 N2 竞速面(会话装载赶不上点击)+内存按访问付费。文件:`crates/auto-lang/src/back_proxy.rs`+`rust_ui.rs` 一行。验证:lazy e2e 新测+五家语料探针翻 lazy 全绿+既有 e2e 回归。AC:AC-09。
  [✅ 已完成 2026-09-21] 实施=`BackProxyConfig.lazy_sessions` 档(start 零预 spawning,spec 进只读目录表;eager 档逐字节一致);`ensure_session` 会话表锁内「查目录→spawn→插表」(并发首击串行化防重),请求在 mpsc 排队至装载就绪(30s 窗,装载期数秒充裕);装载失败=既有 degraded 503;`remove_app` 后目录仍在→下一请求自愈重载;`spawn_session` 提取三臂共用;`has_session` 扩义(在册或已载)。画廊消费点置 lazy=true,13 处测试构造点补 false 保持 eager 语义。测试:`http_e2e_back_proxy_lazy_sessions_load_on_demand`(首击装载应答 int 绑定/次击复用模块态/断链坏语料 degraded 503/ghost 404——注:语法级垃圾会被宽容 parser 解析成零路由 404,断链才是真装载失败面)+五家真实语料探针翻 lazy 档(冷代理逐家按需装载全 200=N2 场景确定性镜像)。门禁:back_proxy e2e **12/12**、auto-man 318/320(2 预存红)、cargo tf **3690/3690**。提交 `b9178c9b3`。
  [✅ 补遗 2026-09-21 走查支撑] **routable 会话准入缺失修复**——`back_needs_session` 谓词只认 ~Stream/~Promise/use auto.(纯 CRUD 不建 session 的 VM inproc 思维),P672 条目4 的 `row.fullstack` 臂不覆盖 routable 档:五家路由 demo 从未进过会话清单,API 即打即 `404 unknown app`。**走查数据冻结=双层根因**(此 404 + N2 死亡窗口),rev1 归因只中后者。修复=start_gallery_back_proxy 准入补 `row.routable` 臂;活体验证:代理 11 sessions(6+5),vite→lazy 链 022/018/021/023 首击 200 带真种子,019 裸探针 400=必填 query 契约正确(浏览器端带全 query)。
  [✅ 补遗二 2026-09-21 用户走查] **020-music-player 媒体面修复**——`Http.get_json("/api/media/scan")` 转译为 `fetch('/api/media/scan')` 打宿主根 404(连接服务失败/端口 8320 文案),代理侧真歌单无人消费。修复=改写闭包补三档通用裸 fetch 前缀化臂(020 是 loadable 档,独立于 api-import 臂;幂等安全)。活体验证:store 产物 fetch('/apps/020-music-player/api/media/scan'),媒体扫描返回真歌单。**走查总评(用户)**:大部分 app 打开+后台数据加载成功;031/017 静态面板=658 既有诚实边界。
...
**rev2 复审(2026-09-21)**:T-08 逐项核对——AC-09 四断言全过(首击装载应答+次击复用+坏语料 503+ghost 404);eager 语义零漂移(13 构造点 false+既有 e2e 12/12);画廊消费点已切 lazy。门禁同 rev1 口径全绿(back_proxy 12/12、auto-man 318/320 仅 2 预存红、tf 3690/3690)。**outcome: pass**,status 翻回 reviewed。
- [2026-09-21] rev2 复审预定:T-08 完成后按同门禁(lazy e2e+五家探针+tf)复核后翻回 reviewed。
- [2026-09-21] **合并收据(PLAN-675:r2,completion_kind: delivered)**:stage: merge | outcome: pass | 交付提交=**1d308add2**(master=plan-675-dev,ff-only 零合并提交)。五 checkpoint:①prepared=reviewed 基线 b9178c9b3/8e6c9909e+规范增量(back-proxy.md 参数绑定/lazy 两节、auto-man/project.md 画廊节、plans.md 双行、specs.json P675-1/2)@worktree 8407ea5a2;②landed=rebase master(18/18 重放,唯一冲突=672 T-18 vs T-02 同域,按 Q-6 裁定 --ours 取本计划版)+门禁刷新(back_proxy 12/12、auto-man 319/321 仅 2 预存、tf **3706/3706** 含 PLAN-039 并入测试)+主干烟雾 26/26;old tip b9178c9b3→new 1d308add2(range-diff 空输出,等价性以重放后全门禁刷新证);③ledger_refreshed=specs.json P675-1(reports)/P675-2(reviews)+auto-man 与 runtime plans.md 双行+INDEX 再生(26 projects);④archived=git mv docs/plans/archive/675-routes-in-embed.md+status: archived;⑤**cleaned**=wt-guard 两轮:首轮逮 018/022 探针 stylekit junction ×2(auto gen 所置)→cmd rmdir 单摘(不穿透)后 clean;清仓=auto-lang worktree 移除(测试再生 rust-workspace 产物还原后)+auto-down 兄弟位移除+prune+plan-675-dev 分支删+组目录 rmdir,注册表/worktree list 双清零。**连带落库**:PLAN-672 全部代码提交(ed1d5fadf 止)随本分支入 master(672 全包含实证),其计划簿记归 672 会话;672 档 N5 双修注记已预落。**遗留**(不阻断 reviewed):①落库按 Q-7 序(672 先落→675 摘最终态→落库),merge 动作随两计划协调;②022/018 布局观察=P675-D1;③P672 档注记本次落笔(见下)。
