...
> **终局裁定（2026-08-27 复核归档）**: 📦 归档。工作在 master(`8b5426fac` 等):Phase 1-3(热重载/OS 用户层 keymap/表达式引擎/enabled-if)+ Phase 5 测试债清偿——9 项既有回归根因归零(RC 持有份额×3 族+handler 名剥参,含 §1 开工基点即失败的 plan370 d3/d4/d6/d7),feature 套件 3581/0、无特性 3125/0、矩阵 48/48、plan370_015 12/12。安全挂账已登记债务簿:ARRAY_LEN 弹栈不释放引用操作数(每调用泄漏一个份额)/裸 i32 堆 id 与 TAG_OBJECT 双轨并存(419 遗留议题,收口需一次性迁移)。
