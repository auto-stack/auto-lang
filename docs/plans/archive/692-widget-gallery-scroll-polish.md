...
- 2026-09-23 W-1（走查顺手修复，范围增注）：用户报告 CodeEditor 页 AutoLang 示例空白。根因=vue.rs code_editor 臂对 `content` 只认状态引用（→v-model），字面串被静默丢弃——补单向 `:model-value` 初始值（escape_js_string 转义+双引号 &quot; 实体化保护属性分隔符）。**范围增注**：此为 code_editor 修复非 scroll 范围，因用户在同一验收走查中报告且同属 widgets-gallery 而并入处置；复审如认定应拆分可独立摘出（commit 407cbf593 单文件自洽）。新增 `p692_code_editor_literal_content` 双断言；vue 组 80/81（唯一红=预存基线 a2vue 金样）。**债务候选（infra）**：auto-os `.auto/ui-cache.json` 入库跟踪但缓存键不含生成器版本——生成器升级后旧产物被判新鲜复用，须手工清缓存（本次已清并落库再生态，auto-os fc9df54）；建议缓存键混入生成器版本/hash。实机：AutoLang 示例 208 字符渲染+行号+AutoLang 高亮全过（截图）。

- 2026-09-23 merge（PLAN-692:r5）收据五 checkpoint：**prepared**（rev5 基线=复审 pass@fa524d126 链；canonical diff=scroll-pane.md SD-01/02+command-family.md SD-03+widgets/project.md 行+core.md 再生，worktree 提交 0503b5023/fa524d126）→ **landed**（auto-lang：rebase 13 提交、range-diff 12 全等+1 追加（core.md 再生 fa524d126），ff-only master tip=fa524d126；auto-os：rebase 5 提交 ff-only main tip=20ce5c8；main 检出 ui-cache churn 还原后落地——入库缓存每次运行即 churn 属 W-1 债务面）→ **ledger_refreshed**（specs.json P692-1 tests/P692-2 reviews 外科插入+INDEX 再生 26 projects+KNOWN-DEBT 五则债务候选）→ **archived**（本文件 git mv+status archived，commit 06130c438）→ **cleaned**（wt-guard 三 worktree 全 clean——auto-os gen pnpm junctions 经 cmd rmdir /s /q 整目录清后过闸；auto-lang worktree 移除+plan-692-dev 删除@fa524d126、auto-os worktree 移除+plan-692-dev 删除@20ce5c8、auto-down 只读依赖位移除、组目录 D:/autostack/.wt/lang-692 全清）。completion_kind: delivered。
## 10. 待澄清事项
- 2026-09-23 W-2（走查修复，范围增注）：用户报告 Combobox 四问题（Empty 常显/候选项无 hover 点击/不支持复杂项/命名错位），要求对照 shadcn-vue 官方重实现。**根因链**：①command 八元素的 schema backends 为 web:"none" → 标签从不走 shadcn 组件路径，前端渲染的是 front/components/command.at 遗留手写 stub 家族（div 空壳）——总根源；②registry apply_schema_vue_mappings 只 overlay 既有 spec，schema-only 元素（command 家族无手写 spec）映射落空；③DSL 标签为 kebab（command-input）而 registry 键为下划线 canonical，缺 kebab 别名恒查无。**修复**：schema 八元素翻转 web:component + vue 映射（真身=assets/shadcn-ui/command 官方 Listbox 实现——scaffold 一直在库，唯一断链即后端翻转）；registry 为 schema-only 元素补建 spec + kebab 别名回填；vue.rs command 臂裁剪（query/placeholder 误发射移除）；CommandItem cursor-pointer+hover 兜底；删除遗留 command.at stub 家族；combobox.at 升级（Simple 真交互选择回填 + Complex Items 示例）。**实机验证**：popover 开→input+三候选渲染、空文案隐藏；filter "vu"→仅 Vue、"zzz"→空文案出现；点选 Vue→触发器标签回填"Vue"、cursor pointer；hover 高亮类在案（真实鼠标生效面）。commit auto-lang fba5e1771（含 registry 单测 p692_schema_only_command_family_gets_vue_specs + 生成映射单测 p692_command_family_shadcn_mapping）+ auto-os 4f8f6a3。**遗留债候选**：popover 选中后不自动关闭（需 open 状态绑定，另立）；VM 臂 command 家族仍 iced:none（无回归）。**过程坑**：本 boot 期间进程三次静默死亡（exit 1 无输出、死点漂移、金丝雀存活→疑似并行会话按名 taskkill 误伤），改用改名副本 auto-692-gen.exe 跑通全程（taskkill /F 特征退出码 1 无输出）；改生成器后须删 .auto/ui-cache.json 才重生成（W-1 已记）。
- 2026-09-23 W-3（走查修复）：用户报告首页分区标题（"LAYOUT" 与圆点/计数）未对齐。实测根因：行容器被撑到 68px——脚手架基础样式 `h2 { mt-8 mb-4 }` 漏进 category-section 标题行的 flex（margin 盒参与 flex 居中 → h2 文字相对圆点/计数下坠 8px）。修复=generate_category_section_html 的 h2 补 `m-0`（元素选择器被 Tailwind 工具类特异性反超）。实测：行高 68→20，圆点/文字/计数三点中心完全重合（719=719=719）。commit 16e53607e + auto-os cfe9649（ui-cache 再生态）。注：脚手架 h1-h6 基础 margin 对其它内联标题行的同类影响待观察（本计划只修此报表位）。

- 2026-09-23 review（rev5）：/auto-plan:review 独立复审。stage: review | plan_id: PLAN-692 | plan_revision: 5 | outcome: **pass** | reviewed_commit: auto-lang plan-692-dev `1ad27285d`（base bebd09387，13 提交）| auto-os plan-692-dev `cfe9649`（base 0fbe421，6 提交）| 双 worktree clean | spec_inputs: widgets/scroll-pane.md（SD-01/02 已落 worktree）+ widgets/command-family.md（SD-03 新建）+ widgets/project.md 行 | **门禁**：cargo tf 全量档 --no-fail-fast 5458 跑 5446 过 12 红——对账：musk_vm_track×6=预存基线（musk×6 待认领在案）、ffi_dual_019=在案 flaky（PLAN-689 留档）、projector_counter×1=预存（counter×1 在案）、native_gate_six=master 同败（本复审实测）、a2vue 金样=master 同败（本会话实测）、plan358_stress=flaky（tf 并发负载下单跑双连过）、**docs_gen core_reference=本计划 schema 变更引发→已 DOCS_GEN_UPDATE 再生 core.md（+1 行 size prop）修复，docs_gen 4/4 绿**——零回归；iced 臂 271/271；新增单测 6 枚全绿（scroll 三态映射/size+合并 style/code_editor 字面 content/registry 家族 spec/家族映射）。| **AC 终态**：AC-01..08 过（本会话实机+单测证据，复审记录各条在案）；AC-05/06 经 rev2/rev3/rev4 用户裁定改述/退役；W-1（code_editor 字面 content）/W-2（Combobox shadcn 官方形态，含过滤/空态/点选回填实测）/W-3（首页标题三点重合 719=719=719）走查批全过。| findings: 无阻断项；债候选五则已入 §0（ui-cache 生成器版本键/popover 选中不自动关闭/VM command iced:none/18px 下限观察/h1-h6 margin 内联隐患）| next: **merge**。复审限制声明：本复审与实现在同一会话执行，verdict 依据工件重建（tf 清单、几何实测、提交链）而非执行者转述；IAB 合成事件不能驱动 reka 交互的验证面（拖拽手感）由用户真鼠标确认。
