...
1. **`??` 可选项**（阻塞步骤 8）：缺省顺做（Pratt 表已有,四层臂+语料
   ...
   → **裁定（2026-09-03,W0 实测）:显式延后,P525-3 维持**。实测现状：
   主 a2r 已支持（`a ?? b`→`a.unwrap_or(b)`,nc_probe 实证）;**原生 VM
   codegen 无臂**（`auto run` 静默空输出——比报错更隐蔽）+自举 lib 三件
   （auto/lib codegen.at/engine.at/a2r.at）全无臂。非 May 耦合超预期,
   而是多层 VM opcode/脱糖+镜像的真实特性片,超出"便宜则顺做"量级。
   留档:原生 VM 对不支持表达式的静默空输出行为本身值得独立登记（本次
   不扩面）。KNOWN-DEBT P525-3 行保持,追加本实测注记。
2. **P523-2② 逃生舱**（阻塞步骤 7）：根因若在主 a2r type 发射深层
   （525-1 同片区敏感性）且超小修量级 → 登记转 532 W0 处置,本计划
   其余照收。
   → **不触发**:实测洞已不复现（525 codegen 早注册批顺带清偿,步骤 7
   无垫片复证 10/20）,无遗留工作,不转 532。
3. **api-example 前端红归属**（阻塞步骤 8）：缺省独立处置不入本计划
   （UI 线/独立小件）;若 pac target 链修复顺带揭示同根因再裁定。
   → **维持缺省**:pac target 链修复（resolve 缺失/包名空/无条件
   auto-lang 依赖三缺口）均为最小工程构建管线问题,与 api-example
   前端 codegen strict 校验红（R008 逗号分隔等）不同根因,独立处置。
