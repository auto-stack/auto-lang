...
（`.route("/", handler)` 里 `handler` 被当成未定义变量）。这是 Auto 语言的语义缺陷
（裸函数名在值位置不识别为函数引用），而非 a2r 名称解析问题。

**已分配计划**：**Plan 383（`383-function-reference-as-value.md`）** 负责彻底解决——
让命名函数在值位置（`let f = handler` / `apply(handler)` / `.route("/", handler)`）
自动作为函数引用，对标 Rust/Python 的隐式函数引用语义，VM + a2r 双路径都修。
不再用闭包 `() => handler()` 绕开。
