...
- **二分矩阵**（最小复现，fixture：`crates/auto-lang/tests/fixtures/plan356_minimal_oom.at`）：
...
**遗留 2（已修，2026-07-17，worktree `plan-356-styleif-attr`）**：完整 sidebar 在
`for ident.field` 修复后推进到 offset ~8873，碰到**第三个独立解析错误**：
`style: if .active_tag == tag { } else { }` 无法解析。
**根因**：与遗留 1 同类——`tag` 被词法分析为保留关键字 `TokenKind::Tag`，而通用表达式
原子解析器 `atom()` 只匹配 `TokenKind::Ident`，所以作为 `==` 右操作数的 `tag` 被拒
（"Expected term"）。前两次修复分别只覆盖了 `parse_event_arg` 和 `parse_view_for_loop`，
没覆盖 `if` 条件位置。
**修复**：`atom()` 的标识符入口（`crates/auto-lang/src/parser.rs`）扩展为
`is_kind(Ident) || cur_is_soft_ident()`，使 `tag`/`type`/`move` 等软关键字在**任意**
表达式位置（条件、操作数等）都被当作普通标识符消费。这是比前两次更根本、更正确的修法。
回归测试：`test_soft_keyword_in_if_condition` / `test_soft_keyword_as_condition_value`。

**🎉 完整 sidebar 端到端通过**：`test_plan356_real_sidebar_generates`（原 `#[ignore]`）
现已取消忽略并通过——015-notes 的 200 行 sidebar.at（commit `50307d51`）能完整解析+生成。

三个修复（commit `0ccc9418` / `1175c25c` / `870b79e6`）叠加，覆盖了原 OOM 及其掩盖的两个
独立解析错误。验收标准全部通过（详见 §5）。窄遗留见 §6 末尾。
...
- [x] 最小复现 case（for + style:if + msg/on）不再 OOM
  — `test_plan356_oom_regression_loop_var_as_handler_arg`，瞬间完成（原 11GB+ OOM）。
- [x] 015-notes 的完整 sidebar.at（200 行，含文件夹树）正常生成
  — `test_plan356_real_sidebar_generates`，端到端通过（fixture `plan356_oom_sidebar.at`）。
- [x] 015-notes 的 editor.at（含 autodown_editor）正常生成
  — 历史版 editor.at（commit `929364da`，176 行）实测生成 5604 字节。
- [x] 生成器内存使用：任意 < 500 行的 .at 文件，峰值内存 < 200MB
  — 377 行（sidebar+editor 组合）生成 12.7KB，3ms，O(N) 线性，远低于阈值。
- [x] 生成器时间：任意 < 500 行的 .at 文件，生成时间 < 5 秒
  — 377 行实测 3ms。
- [x] 回归测试覆盖 OOM 触发条件
  — 8 个测试（`crates/auto-lang/src/ui_gen/api.rs`）：OOM 主修复、软关键字（handler
    参数 / if 条件）、`ident.field` 迭代器、完整 sidebar 端到端、正常循环变量不受影响。

---

## 6. 暂时措施（核心修复已完成；源码恢复为后续独立工作）

Plan 356 期间，015-notes 暂用简化版（commit `c65afcce`）：
- sidebar.at：扁平笔记列表（无文件夹分组、无标签筛选）
- editor.at：textarea 替代 autodown_editor
- notes_store.at：无 all_tags 聚合

**Plan 356 的三个 parser 修复已使完整版重新可行**（验收 2、3 证明 200 行 sidebar 与
176 行 autodown_editor editor 都能解析+生成）。但是否把 015-notes 源码切回完整版
（撤销简化、重跑 `auto gen`、验证运行时行为）是**独立的 015-notes 恢复工作**，不在
Plan 356（parser bug 修复）范围内。建议作为单独的小任务执行。

**已知遗留（窄问题，非 sidebar/editor 触发，不阻塞 Plan 356 验收）**：作为 `style:`
属性值、以**裸标识符**为整个条件的 `style: if <ident> { }`（无 `else`、非 `.field`、
非比较）仍无法解析。这是 view-prop 位置 `if` 解析的独立边角问题，015-notes 未用到，
留待后续。
