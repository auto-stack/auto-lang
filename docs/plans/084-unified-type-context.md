...
**统一类型查询 API** (Phase 5 - 额外完成)

- ✅ TypeStore 添加 `lookup_fn_decl()` / `lookup_fn_decl_str()` 方法
- ✅ TypeStore 添加 `lookup_spec_decl()` / `lookup_spec_decl_str()` 方法
- ✅ InferenceContext 的 `lookup_type_decl()` 优先使用 TypeStore
- ✅ InferenceContext 的 `lookup_meta()` 优先使用 TypeStore
- ✅ Parser 的 `lookup_meta()` 优先使用 TypeStore
- ✅ Parser 的 `lookup_type()` 优先使用 TypeStore

**架构优势：**
- 单一数据源：所有类型信息存储在 TypeStore 中
- 共享访问：通过 `Arc<TypeStore>` 实现跨组件共享
- 向后兼容：保留现有注册表作为回退
- 统一查询：所有组件通过相同的 API 查询类型信息

**类型查询优先级：**
1. TypeStore（类型/函数/Spec 声明）
2. InferenceContext（变量绑定）
3. Universe（向后兼容回退）
