...
**状态**: Phase 1-9, 11 完成；Phase 10/12/13 暂停（需 VM 架构改动）；cookbook 测试 124/124 pass；61 个 SIMPLIFIED .at 已去桩化
...
## 11. 独立编译验证 v2（2026-05-12 最终）

修复 BUG-A/B/C 后，重新运行 batch compilation。
...
#### 类别 A：a2r Transpiler 可修复（~55 个）

| # | 根因 | 错误码 | 数量 | 典型案例 | 修复难度 |
|---|------|--------|------|----------|----------|
| A1 | crate 名作为值使用（`fs.read()` → 应为 `std::fs::read()`） | E0423 | ~15 | file/001-014, encoding/001, os/002 | 中 |
| A2 | 类型不匹配（i32 代替复杂类型、泛型缺失） | E0308 | ~20 | encoding/003-012, errors/001, concurrency/001-003 | 高 |
| A3 | 迭代器元素类型错误（DirEntry → i32） | E0599 | ~5 | file/005-012 `file_name on i32` | 中 |
| A4 | 类型标注缺失 | E0282/3/4 | ~10 | file/002-004, concurrency/008, text/004-006 | 中 |
| A5 | Display trait 遗漏（Option/DateTime/Vec） | E0277 | ~5 | datetime/002, concurrency/005-006, mem/001 | 低 |
...
| B2 | regex 字面量语法 `r"..."` 未转译 | E0422 | ~5 | text/001-006 `cannot find struct r` | 高 |
| B3 | byte 字面量语法 `b"..."` 未转译 | E0422 | ~2 | os/006, cryptography/001 `cannot find struct b` | 中 |

#### 类别 C：基础设施/Mock 限制（~18 个）

| # | 根因 | 错误码 | 数量 | 典型案例 | 说明 |
|---|------|--------|------|----------|------|
| C1 | 外部 crate 未安装（batch_compile.sh 缺少 dep） | E0432 | ~30 | rand(8), log(5), regex(4), semver(3) | mock 无法提供真实 API |
| C2 | 未声明类型（Auto `List`/`File`/`Normal`） | E0433 | ~8 | compression, concurrency, devtools | Auto 类型未映射到 Rust |
| C3 | a2r_std 缺少函数（`str_ends_with`, `get_or`） | E0425 | ~4 | file/006-009, os/001 | mock stdlib 不完整 |
| C4 | 其他（`network` 未定义、Split 不可索引等） | 混合 | ~4 | devtools/007, text/007 | 个别案例 |

> **注**：C1 中的 ~30 个 E0432 错误并非 a2r bug — batch_compile.sh 的 Cargo.toml 未包含这些依赖。如果使用真实的 Cargo.toml（含所有 deps），这些错误会消失，但会暴露更深层的类型/API 映射问题。因此 C1 的真实影响小于表面数字。

### 修复优先级建议

1. **A1: crate 名作为值** — 影响 ~15 个案例，修复后可大幅提升通过率
2. **A5: Display trait 遗漏** — 影响 ~5 个案例，难度低
3. **A3: 迭代器元素类型** — 影响 ~5 个案例，与 A1 相关
4. **B2/B3: 字面量语法** — regex/byte 字面量转译，影响 ~7 个案例
5. **A2: 类型不匹配** — 影响 ~20 个案例，但难度最高（需完善类型系统映射）
