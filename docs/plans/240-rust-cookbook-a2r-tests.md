...
**状态**: Phase 8 完成（163/163 AutoVM 通过 + 60+ 文件 Auto 语法重写）
...
| 3 | **VM String 返回值 bug** | `File.read_text` 返回的 String 通过 `.?` 后 print 显示为负值 |

## 12. Tier C 文件状态与未来升级计划

### 12.1 全功能版（17 个）— 执行真实操作
...
| 文件 | 当前演示 | 升级需要 |
|------|----------|----------|
| `net/001_listen_unused.at` | TcpListener 演示 | `std::net::TcpListener` shim |
...
| 文件 | 当前演示 | 升级需要 |
|------|----------|----------|
| `web/clients/api/001_rest_get.at` | 模拟 REST GET | `dep reqwest` + FFI + async |
| `web/clients/api/002_rest_post.at` | 模拟 REST POST | `dep reqwest` + FFI + async |
| `web/clients/api/003_rest_head.at` | 模拟 REST HEAD | `dep reqwest` + FFI + async |
| `web/clients/api/004_paginated.at` | 模拟分页请求 | `dep reqwest` + FFI + async |
| `web/clients/authentication/001_basic.at` | 模拟 Basic Auth | `dep reqwest` + FFI + async |
| `web/clients/download/001_basic.at` | 模拟文件下载 | `dep reqwest` + FFI + async |
| `web/clients/download/002_partial.at` | 模拟分段下载 | `dep reqwest` + FFI + async |
| `web/clients/download/003_post_file.at` | 模拟文件上传 | `dep reqwest` + FFI + async |
| `web/clients/requests/002_get_async.at` | 模拟异步 GET | `dep reqwest` + FFI + async |

#### web/scraping（3 个）

| 文件 | 当前演示 | 升级需要 |
|------|----------|----------|
| `web/scraping/001_extract_links.at` | 模拟链接提取 | `dep scraper` + FFI |
| `web/scraping/002_unique.at` | 模拟去重链接 | `dep scraper` + FFI |
| `web/scraping/003_broken.at` | 模拟断链检测 | `dep reqwest` + `dep scraper` + FFI + async |

#### web/mime（2 个）

| 文件 | 当前演示 | 升级需要 |
|------|----------|----------|
| `web/mime/001_filename/filename.at` | MIME 推断演示 | `dep mime_guess` + FFI |
| `web/mime/001_request.at` | MIME 请求演示 | `dep mime` + FFI + HTTP |
| `web/mime/002_string/string.at` | MIME 解析演示 | `dep mime` + FFI |

#### web/url（5 个）

| 文件 | 当前演示 | 升级需要 |
|------|----------|----------|
| `web/url/001_base/base.at` | 简化 URL 解析 | `dep url` + native shim（部分已有 opaque 支持） |
| `web/url/002_parse/parse.at` | 简化查询参数 | `dep url` + native shim |
| `web/url/003_fragment/fragment.at` | 简化片段提取 | `dep url` + native shim |
| `web/url/004_new/new.at` | 简化 URL 拼接 | `dep url` + native shim |
| `web/url/005_origin/origin.at` | 简化 origin 提取 | `dep url` + native shim |

### 12.3 升级路线图

**Phase 9: URL crate 全功能化**（5 个文件，低难度）
- `url` crate 已有 opaque type 支持（`auto.url_opaque.*`）
- 将 cookbook 文件从简化版改为调用真实 URL API
...
**Phase 11: 网络栈**（14 个文件，中-高难度）
- `auto.http.get/post` 已有实现，修复 tokio runtime 冲突
- `dep reqwest` FFI 桥接
- `dep scraper` FFI 桥接
- 9 个 web/clients + 3 个 scraping + 1 个 mime/request + 1 个 net → 全功能
...
完成 async 执行器 + FFI 桥接可一次性解锁 ~30 个文件（14 网络 + 6 数据库 + 7 async + 3 scraping + 1 memmap）。
