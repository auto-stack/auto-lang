...
| # | 问题 | 文件数 | 阻塞原因 |
|---|------|--------|----------|
| 1 | **无网络栈 (HTTP/TCP)** | 14 | 需 `dep` + `use.rust` FFI 桥接 (Plan 092) 或 stdlib 扩展 |
| 2 | **无数据库支持 (SQLite/Postgres)** | 6 | 需 FFI 桥接 + async 执行器 |
| 3 | **无 async 执行器** | 5 | VM 架构级：需内嵌 tokio runtime 或 async scheduler |
| 4 | **Channel 未暴露** | 2 | 被 async 执行器阻塞 |
| 5 | **无 mmap** | 1 | 需 FFI 或 memmap2 builtin |
| 6 | **无 C/C++ 构建** | 3 | 需 build-time codegen |
| 7 | **无 MIME 解析** | 1 | 需 `dep mime` + FFI |
| 8 | **VM String 返回值 bug** | ~4 | `File.read_text` 返回的 `Result<String,String>` 通过 `.?` 后 print 显示为负值，需 VM 修复 |

### 最大杠杆点

完成 Plan 092 的 `dep` + `use.rust` FFI 桥接可一次性解锁 ~24 个文件（14 网络 + 6 数据库 + 1 mmap + 1 MIME + 其他）。
