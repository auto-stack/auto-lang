...
## 12. VM Cookbook Batch 测试（2026-05-13）
...
| # | 测试 | 错误 | 根因 | 可修复性 |
|---|------|------|------|----------|
| 1 | algorithms/011_rand_dist | `Unknown Rust stdlib call: new.Normal` | cargo 增量编译缓存：已验证新 binary 中 Normal.new 正常路由，batch 脚本可能读到旧 binary | — |
| 2 | compression/003_tar_strip_prefix | `File.open failed: os error 2` | 运行时缺少测试文件（tar extract 产物未找到） | — |
| 3 | concurrency/005_rayon_iter_mut | `codegen.rs:6939 unimplemented!` | `Expr::Lambda` 未实现（`x => { x * 2 }` 闭包） | 需 codegen 支持 Lambda |
| 4 | concurrency/007_crossbeam_complex | `codegen.rs:6939 unimplemented!` | 同上：闭包语法未实现 | 需 codegen 支持 Lambda |
| 5 | concurrency/008_crossbeam_spawn | `codegen.rs:6939 unimplemented!` | 同上 | 需 codegen 支持 Lambda |
| 6 | concurrency/009_global_mut_state | `codegen.rs:6939 unimplemented!` | 同上 | 需 codegen 支持 Lambda |
| 7 | datetime/002_checked | chrono lib.rs:717 panic | `chrono::DateTime::checked_add` 在 nanbox 模式下 panic（chrono 内部 unwrap 失败） | 需 chrono wrapper |
| 8 | devtools/001_log_debug | `Undefined variable: env_logger` | `env_logger` 已加入 stdlib module list，但 `dep` 声明可能未注册到变量表 | 需 dep→变量注册 |
| 9 | devtools/004_log_custom | `Undefined variable: log` | 同上 | 需 dep→变量注册 |
| 10 | devtools/007_log_mod | `Undefined variable: env_logger` | 同 #8 | 需 dep→变量注册 |
| 11 | devtools/008_log_timestamp | `codegen.rs:6939 unimplemented!` | 闭包语法未实现 | 需 codegen 支持 Lambda |
| 12 | errors/002_anyhow | 运行后挂起（无输出） | anyhow 错误处理链未完整实现 | 需 anyhow dispatch |
| 13 | file/004_modified | `no function 'None.filter_map'` | `Option` 上调用了 `filter_map`（a2r 转译结果的问题） | a2r 已修复 |
| 14 | os/003_error_file | 运行后挂起 | Command.stderr 输出读取阻塞 | 需 OS dispatch 修复 |
| 15 | safety/001_heapless | `Invalid Rust stdlib handle (handle=0)` | `heapless::Vec` 未注册 dispatch handler | 需 heapless dispatch |
| 16 | versioning/005_semver_complex | `Version::parse failed: unexpected '>'` | `VersionReq.parse` 应调用 `semver::VersionReq::parse` | 需 semver dispatch |

### 12.4 14 个 TIMEOUT 分类

| # | 测试 | 原因 |
|---|------|------|
| 1 | datetime/001_elapsed_time | Instant.elapsed 循环太慢（VM dispatch 开销大） |
| 2 | encoding/003_csv_read | CSV Reader.records() for 循环无限（iterator 机制与 for-in 不匹配） |
| 3 | encoding/007_csv_delimiter | 同上 |
| 4 | encoding/008_csv_filter | 同上 |
| 5 | encoding/009_csv_invalid | 同上 |
| 6 | encoding/012_csv_transform | 同上 |
| 7 | file/002_find_files | walkdir for 循环超时 |
| 8 | file/003_recursive_size | 同上 |
| 9 | file/008_loops | 嵌套 for 循环超时 |
| 10 | file/010_recursive | 递归 walkdir 超时 |
| 11 | file/014_read_lines_temp | fs 操作超时 |
| 12 | text/003_regex_hashtags | regex 迭代器 for 循环超时 |
| 13 | text/005_filter_log | 同上 |
| 14 | text/006_phone | 同上 |

### 12.5 根因分类

| 根因 | FAIL 数 | TIMEOUT 数 | 总计 | 修复方向 |
|------|---------|-----------|------|----------|
| 闭包/Lambda 未实现 | 5 | 0 | 5 | codegen.rs 添加 `Expr::Lambda` 支持 |
| dep 声明未注册到变量表 | 3 | 0 | 3 | dep→变量注册机制 |
| CSV iterator for-in 不匹配 | 0 | 5 | 5 | for-in + iterator 机制修复 |
| 外部 crate dispatch handler 缺失 | 3 | 0 | 3 | 添加 shim（heapless, semver, anyhow） |
| walkdir/regex iterator 超时 | 0 | 5 | 5 | 同 iterator 问题 |
| chrono 内部 panic | 1 | 0 | 1 | chrono wrapper |
| 运行时文件缺失 | 1 | 0 | 1 | — |
| OS dispatch 阻塞 | 1 | 1 | 2 | OS dispatch 修复 |
| cargo 增量编译缓存 | 1 | 0 | 1 | — |
...
| 优先级 | 任务 | 影响 | 难度 |
|--------|------|------|------|
| P1 | for-in + iterator 机制修复 | 10 TIMEOUT | 高 |
| P2 | codegen 添加 Expr::Lambda 支持 | 5 FAIL | 高 |
| P3 | dep 声明自动注册到变量表 | 3 FAIL | 中 |
| P4 | 添加缺失的 dispatch handler（heapless, semver, anyhow） | 3 FAIL | 低 |
| P5 | chrono wrapper 避免 panic | 1 FAIL | 低 |
| P6 | OS dispatch 阻塞修复 | 2 | 中 |
