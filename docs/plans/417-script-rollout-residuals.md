...
| E3 TRAIT-VM-1 | trait 默认方法 VM 分歧 | ✅ 2026-08-22 完成(有界泛型:parser `has` 语法 + VM CALL_SPEC 动态分派 + CallFrame/字符串重映射两处隐藏 bug 修复 + a2r `<T: Spec>` 发射;trait_advanced 三方 14/14) | 2-3 天 |
...
**顺序**: E1 ✅ → E5 ✅ → E4 ✅ → E3 ✅(TRAIT-VM-1 有界泛型,2026-08-22 修复:`has` 语法 + CALL_SPEC 运行时类型分派 + a2r `<T: Spec>`;详见 known-divergences.md DIV-TRAIT-VM-1 翻转条)→ 剩 E2(LANG-1 关联类型,语言级)。Phase E 五项已关四项。每项落地后 known-divergences.md 对应条目 status → fixed。
