...
feature_name: auto-os 立项 Stage A——伞形仓骨架 + 首个真实 app「计划看板」（只读五态）
...
# [PLAN-579] auto-os 立项 Stage A——伞形仓骨架 + 首个真实 app「计划看板」
...
2. **首个真实 app「计划看板」**（`D:/autostack/auto-plans`，新建 git 仓）：
   只读 AutoUI 看板，扫描 auto-lang `docs/plans/*.md` + `docs/plans/archive/*.md`
   的 YAML frontmatter，按五态状态机（drafting → executing → execution_done →
   reviewed → archived）映射看板列，卡片显示 plan_id / 标题 / current_step
   进度 / updated_at。Vue + VM 双端。

**明确不做**（边界）：

- 不做写回——状态翻转归 auto-plan 四技能所有，看板是仪表盘不是编辑器；
...
2. `D:/autostack/auto-plans` 仓就绪：examples 形态的 AutoUI app（pac.at +
   src/front + src/back + tests），`auto run`（Vue）与 `auto run -r vm`（VM）
   双端跑通。
3. 看板数据真实性验收：对账真实 auto-lang 检出——看板各列卡片数之和 ==
   `ls docs/plans/*.md docs/plans/archive/*.md` 文件数（活账对账，见验收标准 C3）。
4. 结构互验：新 app 仓从零在仓外被 auto CLI 完整拉起（gen → run → test），
...
│   └── docs/plans/*.md          # ← 看板数据源（frontmatter 只读）
...
└── auto-plans/                  # ★ 新建：首个真实 app 仓（计划看板）
    ├── pac.at                   #   仓根（examples 形态；T6 验证不过则转 auto/ 形态）
    ├── src/front/               #   app.at + plans_store.at + pages/board.at
    ├── src/back/                #   api.at（#[api] 端点）+ plans_scanner.at（扫描解析）
...
```
[auto-plans front: board]  ←HTTP←  GET /api/plans, GET /api/meta
[auto-plans back: api.at]  →  plans_scanner.at
    resolve_lang_root():  Env.get("AUTO_LANG_ROOT") → "../auto-lang"(file.exists) → "D:/autostack/auto-lang"
    fs.read_dir(docs/plans) + fs.read_dir(docs/plans/archive)   # 返回 JSON 文件名数组
    File.read_text(每个 .md) → parse_frontmatter() → status→column 映射 → []Plan
```
...
- **后端 .at 能力（均已核实存在，双后端同签名）**：`fs.read_dir(dir) str`（返回
  ...
  reading.at:97 用例）、`json` 模块（解析 read_dir 结果）。
...
1. **需求是真实的**：auto-lang 已周期性人工盘点计划状态（archive 内
   ...
   手工 git grep 维持。看板 = 把这个动作产品化。
2. **数据源规整可浅解析**：v2 frontmatter 字段固定（plan_id/status/
...
5. **submodule 否决依据**（用户接受的三阶段裁定）：现有扁平兄弟仓 + 跨仓
...
6. **examples 022-kanban 不是复用底座**：其定位是教学示例（store 模式/类型化
   后端/拖拽演示），CRUD 与拖拽在只读看板里无位置；本计划新写，仅复用其
   结构模式（k1 store + 单集合列过滤渲染）。

## 详细设计

### 后端：`src/back/plans_scanner.at`（纯逻辑 + 扫描）

```auto
// 路径解析序（与 AGENTS.md 跨仓约定一致）
fn resolve_lang_root() str:
    1. Env.get("AUTO_LANG_ROOT") 非空且 file.exists(join(root, "docs/plans")) → 用之
...
fn parse_frontmatter(text str) Map/struct:
    逐行扫描：首行 == "---" 进入，遇第二个 "---" 退出
...
// 扫描 + 映射
pub fn scan_plans(root str) List<Plan>:
...
```auto
pub type Plan = {
    plan_id: str, file: str, title: str,
    status: str, column: str, badge: str,        // badge: "" | "parked"
    current_step: int, total_step: int,
    updated_at: str, archived: bool,
}

#[api(GET /api/plans)]  pub fn all_plans() []Plan     // 全量（含 archived），前端列过滤
#[api(GET /api/meta)]   pub fn meta() Meta            // { source_root, scanned_at, count_active, count_archived }
```

### 前端

- `plans_store.at`（k1 模式）：`PlansStore { plans List<Plan>, meta Meta, load() }`。
- `app.at`：路由壳 `routes{ "/" -> board }` + header（app 名 + 数据源路径 +
  扫描时间 + 刷新按钮 → store.load()）。
- `pages/board.at`：四列横排（drafting/executing/execution_done/reviewed，
  `for plan in .store.plans` + `if plan.column == ...` 过滤，022 同型）；卡片
  = plan_id 徽章 + title（超长截断）+ 进度条（total>0 时 current/total 文本
  + 百分比条，否则"—"）+ updated_at + badge（parked 灰标）；底部 archived
  折叠行：总数 + 最近 5 张缩略（点击展开完整列表，v1 用显隐切换即可）。
- 空态：meta.source_root 为错误串时显示引导文案（设 AUTO_LANG_ROOT 或将
  auto-lang 检出置于兄弟位置）。

### fixture 合成语料（`tests/testdata/fake-lang/`）
...
playwright 与双端验证均以 `AUTO_LANG_ROOT=<repo>/tests/testdata/fake-lang`
注入（同时验证解析序第 1 优先级），断言全确定性。
...
- `apps.manifest`：`{ "apps": [ { "id": "plans", "name": "计划看板",
  "repo": "../auto-plans", "kind": "repo", "ports": [17100, 17101],
...
| 层 | 内容 | 判定 |
|---|---|---|
| P1 解析对拍 | scanner 对 fixture 六文件输出（列映射/进度默认/parked 徽章/stem 兜底） | 经 /api/plans curl 断言 |
| P2 playwright | T1 四列标题渲染；T2 fixture 卡片计数（1/1/2/0 列 + archived=2）；T3 进度条与"—"空态；T4 parked 徽章；T5 archived 折叠展开；T6 刷新按钮重 fetch；T7 无 console error | tests/ 套件全绿（AUTO_LANG_ROOT 指向 fixture） |
| P3 双端一致 | autoui-verifier：vue（test_vue_playwright.mjs）+ vm（test_vm_mcp.py）各一轮 | 两端截图/交互断言一致 |
| P4 真实数据对账 | AUTO_LANG_ROOT 不设，跑真实 auto-lang 检出；`ls docs/plans/*.md docs/plans/archive/*.md \| wc -l` == 看板总卡数 | 数目相等（P4 截图入 evidence） |
...
- **C1 仓骨架**：auto-os 五件套齐（README/AGENTS.md/apps.manifest/plan 脚手架
  ...
  （验证后删除产物、回滚 .next-id）。
- **C2 双端跑通**：auto-plans 在 Vue 轨（:17100）与 VM 轨（-r vm）均渲染
  fixture 数据；P2 套件 7 测试全绿；P3 双端一致性记录入本文件复审节。
- **C3 活账对账**：真实数据下，看板全部卡片数（四列 + archived 计数）==
...
- **C5 互链**：auto-os README Apps 表含 plans 行并链 ../auto-plans；auto-plans
  README 注明伞形归属与数据源解析序。
...
- [ ] **T4 auto-os apps.manifest**
  写 `apps.manifest`（详细设计节 JSON，含 plans 一行）；README Apps 表同步该行。
  验证：`python -c "import json;json.load(open('D:/autostack/auto-os/apps.manifest'));print('ok')"`。
- [ ] **T5 建仓 auto-plans 骨架**
  `git init -b main D:/autostack/auto-plans`；写 `README.md`（首个真实 app 定位、
  数据源解析序、运行/测试方式、伞形归属反链）、`.gitignore`（`node_modules/
  gen/ dist/ target/ *.log`）、`pac.at`（`name: "auto-plans"`、`scene: "ui"`、
  `front_port: 17100`、`back_port: 17101`，参 022/028 字段集）；建目录
  `src/front/pages/ src/back/ tests/`。首提交。
  验证：`git -C D:/autostack/auto-plans log --oneline` ≥1 行。
- [ ] **T6 仓外 CLI 可用性验证（关键前置）**
  在 `D:/autostack/auto-plans` 放最小 front（hello 级 `src/front/app.at`）后跑
...
- [ ] **T7 后端类型与端点骨架**
  写 `src/back/api.at`：`pub type Plan`、`pub type Meta`（字段见详细设计）、
  `#[api(GET /api/plans)] all_plans()` 与 `#[api(GET /api/meta)] meta()` 骨架
  （暂返空集合）。写 `src/back/plans_scanner.at` 空壳（仅模块头注释）。
  验证：`auto gen` 重跑退出码 0（类型/路由代码生成通过）。
- [ ] **T8 frontmatter 浅解析器**
  写 `src/back/plans_scanner.at` 的 `parse_frontmatter(text str)`（详细设计
  算法：定界扫描/key: value/剥 ` #` 注释/`[` 列表跳过/缺省值），并为 fixture
  单文件写临时驱动（scanner 内 `pub fn selftest_parse() str` 返回拼接结果，
  供 curl 观察，T14 落 playwright 后删除）。
  验证：`auto run`（Vue 轨，AUTO_LANG_ROOT 指向 T14 前可先用 auto-lang 真实
  检出）后 `curl -s http://127.0.0.1:17101/api/plans` 返回 JSON 数组且
  PLAN-577 等真实 plan_id 在列。
- [ ] **T9 路径解析序 + 扫描 + 状态映射**
  写 `resolve_lang_root()`（三级序 + exists 校验 + 错误串）与
  `scan_plans(root)`（read_dir 双目录/json 解析/.md 过滤/read_text/映射
  column/status→column 表 + parked 徽章 + title=stem 兜底 + archived 位）；
  api.at 两端点接线（meta 返回 source_root/scanned_at/计数）。a2r 约定：
  返回 struct 走 `var result` + `let` ctor。
  验证：`curl -s :17101/api/meta` source_root 指向真实检出；
  `AUTO_LANG_ROOT=D:/nonexistent curl` 场景降级到兄弟/主检出路径序仍正确。
- [ ] **T10 前端 store**
  写 `src/front/plans_store.at`：`PlansStore { plans List[Plan], meta Meta,
  load() }`，load 经 `use back.api` 拉两端点（k1 模式，参 022 board_store）。
  验证：`auto gen` 通过（front 引 back 类型可解析）。
- [ ] **T11 前端壳与路由**
  写 `src/front/app.at`：header（标题"AutoOS Plans · 计划看板" + 数据源路径 +
  扫描时间 + 刷新按钮）+ `routes{ "/" -> board }`；占位 board 页。
  验证：`auto run` 打开 :17100 见 header 与空板。
- [ ] **T12 board 列与卡片渲染**
  写 `src/front/pages/board.at`：四列横排（列头 drafting/executing/
  execution_done/reviewed + 计数徽章）；卡片 = plan_id 徽章 + title 截断 +
  进度条（total>0：`current/total` + 百分比；否则"—"）+ updated_at +
  parked 灰标（`for` + `if plan.column ==` 过滤，022 同型）。
  验证：浏览器人工目检真实数据四列卡片渲染正确（截图存
  `D:/autostack/auto-plans/screenshots/t12-board.png`）。
- [ ] **T13 archived 折叠与空态**
...
- [ ] **T14 fixture 合成语料**
  ...
  `docs/plans/archive/{091,092}-fixture-archived*.md`（详细设计节六文件规格，
  覆盖 executing 进度/drafting/legacy draft/缺步数字段/archived×2 排序）；
  删 T8 临时 selftest 驱动。
  验证：`AUTO_LANG_ROOT=<abs>/tests/testdata/fake-lang curl -s :17101/api/plans`
  六条且列映射符合设计表。
- [ ] **T15 playwright 套件**
  `tests/` 建 playwright（package.json + spec，沿 022 形态）：T1 四列标题；
  T2 计数 1/1/2/0 + archived 2；T3 进度条与 "—"；T4 parked 徽章；T5 折叠展开；
  T6 刷新重 fetch；T7 零 console error。启动脚本注入 AUTO_LANG_ROOT=fixture。
  验证：`cd D:/autostack/auto-plans/tests && npm install && npm test` 全绿。
- [ ] **T16 双端一致性验证**
  autoui-verifier：Vue 轨（AUTO_LANG_ROOT=fixture，:17100）+
  `test_vue_playwright.mjs`；VM 轨 `auto run -r vm` + `test_vm_mcp.py`；
  真实数据各补一轮（P4 对账）。
  验证：双端断言绿；`ls D:/autostack/auto-lang/docs/plans/*.md
  D:/autostack/auto-lang/docs/plans/archive/*.md | wc -l` == 看板总卡数。
- [ ] **T17 伞形终登记与互链**
  复核 `apps.manifest` ports/status 与实测一致；auto-os README Apps 表与
  auto-plans README 反链定稿；两仓分别提交
  （`feat(app): plans 计划看板 v1 只读双端 (Plan 579)` /
  `feat(umbrella): 登记 plans 首个真实 app (Plan 579)`）。
  验证：C5 互链 grep 通过。
- [ ] **T18 收尾健康检查**
...
1. **app 仓命名**：默认 `auto-plans`（沿 auto-* 扁平命名）；备选 `auto-board`
   / `auto-kanban`。若用户另定名，T5 起全局替换，无结构性影响。
2. **pac.at 仓根形态**（T6 分支点）：examples 形态（仓根）优先；若 auto CLI
   仓外仅支持 auto-os-config 的 `auto/` 子目录形态，则按先例适配并在 T6 证据
   行记录——属形态选择，不登记 KNOWN-DEBT。
3. **VM 轨 iced 端口**：VM 模式无 vite proxy，:17100/:17101 直连形态以
   autoui-verifier 实测为准，若 VM 轨端口注入存在 examples 时代已知缺口
   （022 README 端口注记），以 env 覆盖同法处置并在 T16 证据行注记。
4. **Stage B 启动条件**（非本计划）：在途 525/526 折叠 + 541/566/576/577
