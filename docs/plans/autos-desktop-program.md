...
| **509**（原提案 457）Smithay 宿主 Stage 1 | M5 | Linux 原生合成器宿主，复用桌面 shell——路线评估定案（auto-cosmic 复活/libcosmic/Smithay+桌面协议宿主三路线）+ 最小骨架 + shell 首帧 + I1 零分叉核对（`509-smithay-host-stage1.md`） | 🔄 executing 2026-09-01（T1 裁定 **B**：Smithay+桌面协议宿主，smithay 0.7.0 入 manifest——报告 `509-smithay-route-verdict.md`；T5 环境 = WSL2+WSLg，基线成文；T2 双平台编译绿（host-smithay 新 crate）；T4 合成循环实跑 + 像素证据；T3 shell 首帧上屏（生产链渲染→宿主纹理）+ I1 diff 干净（auto-lang 仅 3 文件 cfg 差异）；live attach 登记 Stage 2） | 462+463 ✅ |
