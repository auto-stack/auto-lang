...
| B12 | B10(b) 副产品 | 013-todo VM 缺口（2026-08-22 三轮收官）：**(i) ✅ 幻影实参帧错位已修**（695b4736：handler 参数数门控 + 复现×3）。**(ii) ✅ 已修**（`plan-fix/b12-write2` 合并 41c9ac39）——backtrace 直捕定案：第二次 todos 覆写发射者是 iced update 循环内嵌的**史前 demo 遗留 Todo 应用**（'Dynamic Todo List helpers'）：独立 Rust 侧 state.todos（启动为空）+ sync_todos_to_vm 把伪造 `Array([Obj{id: 枚举索引}])` 写回碾碎真实 List<Todo> 句柄（种子丢失+id 恒 0 双谜底；"重建未跑 module_init/全局清空"两早期理论均已被插桩退役）；修复=legacy 臂仅在 `!has_handler(base)` 时兜底。**实机 013 desktop_mcp 12→15 过/0 败**：T6 添加流三连全过、T7 toggle-all 真语义通过。**(a) 仍开放**：Init 计数循环 `.todos[i].done` 对 VmRef 元素取字段失效（active_count=0≠3，VM 元素访问缺口，headless 同现）；**(iii)** 子组件事件剥离（D-GAP-4 家族）留档 | (i)(ii) 已关闭；(a) 待修（VM VmRef 元素字段访问）；(iii) 留档 |
