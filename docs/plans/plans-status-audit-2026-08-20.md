...
| B12 | B10(b) 副产品 | 013-todo VM 缺口（2026-08-22 二轮精化，`plan-fix/b12-reload`）：**(i) ✅ 幻影实参帧错位已修**（695b4736：handler 参数数门控 + 复现×3，stash 红→绿实证）。**(ii) 残余精化**——重建/全局清空两论均退役（fire_init 一次、bridge 地址差=move、LOAD nextid=4/todos=H 实证全局完好且第 1 次 `.todos=H` 写入正确=幻影修复生效）；**真凶收窄为同一次 AddTodo 的第二次 todos 覆写**：set_field(todos) 连续两次——第 2 次 `Array([Obj{id:0,text,done}])` 来源未明（Obj 式构造 id=0 与 headless 构造保真 id=nextid 矛盾，疑 GUI 派发链的二次执行/转换路径）；下案入口=同轮插桩 SET_FIELD+CALL_SPEC to_array+CONSTRUCT 定位第 2 次写发射者（探针脚本模式已验证）。合成复现已扩展断言（构造字段保真+计数，全绿，efd02e82）。**(iii)** 子组件事件剥离（D-GAP-4 家族）留档 | (i) 已关闭；(ii) 待下案（第 2 次写发射者定位）；(iii) 留档 |
