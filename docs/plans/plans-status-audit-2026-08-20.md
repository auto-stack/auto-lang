...
| ID | 来源 | 任务 | 暂缓原因 |
|---|---|---|---|
| B1 | 405 | 023-realworld 真 token 认证（current_user 空桩） | 应用级功能，需 api/db/playwright 联动改造，建议独立小计划。**2026-08-21 scoping 核实**：服务端管线已就绪——build_handler_args 已把 Authorization 头经 meta 参数 `{"cookies":…,"auth":…}` 注入 handler（http_server.rs:2042），Plan 352 中间件链 + 346/B6 的 request_info（含 request_id）也可用；剩余工作纯 023 应用层：db 用户表 + login/register 发 token + 各端点鉴权检查 + playwright e2e | 待独立计划（管线无缺口，纯应用层） |
...
| B6 | 346 剩余 | 服务端 multipart 上传、Rate Limit、Request-ID | **Request-ID + Rate Limit ✅ 2026-08-21 已修**（`plan-fix/b6-http` 合并见 git log）：X-Request-Id 透传/铸造 + 全响应路径回显（含中间件 request_info 与日志行）；`http.rate_limit(n, ms)` 原生（catalog 3109）每 IP 固定窗口，超限 429+Retry-After，clear_http_routes 顺带重置限流器（e2e 隔离）；e2e ×3 全绿。**multipart 上传仍待做**（RFC 2045 boundary 解析 + handler 文件参数，独立批次） | Rate Limit + Request-ID 已关闭；multipart 待办 |
...
| B12 | B10(b) 副产品 | 013-todo VM 模式三处 VM 语义缺口（2026-08-21 实机实证 + **同日五轮插桩/disasm 根因调查**，从"三个谜"收敛为两个已定位缺陷）：**(a)** store.Init 计数循环 `.todos[i].done` 对 VmRef 数组元素取字段全非 false → `active_count=0`——疑与 (b2) 同族（构造的字段值丢失）；**(b)** `.todos = list_todos()` 在 AddTodo 中写入 `Array([Obj{id:0}])` 而非 List 句柄——**已定位 ABI 错位**：nanbox 字符串占 2 槽（encode_string+encode_null），SET_FIELD 的固定 3-pop 与之错位（实测第二 pop 弹出字符串负标签 -175 → obj id 成垃圾 → 写入静默落空）；伴随 (b2) `Todo { id: nextid }` 的 LOAD_GLOBAL 来源字段在构造中丢失（id=0、text 参数存活）。**已排除**：模块全局生命周期正常（单 VM，地址差为 move 重定位；module_init 一次、globals 持久）；shim_list_push 原位追加正常（H=4000006 种子 4 条+probe）；to_array 恒等正常；测试计数器曾把对象内逗号当分隔（"4→1 vs 4→3 非确定性"系误报，恒为 1 条）。根治 = codegen/SET_FIELD 字符串槽位 ABI 手术 + 构造字段来源修复，回归面覆盖全量 VM 套件，**须独立小计划**（复现探针：013 type+submit + LOAD/STORE_GLOBAL/SET_FIELD/set_field/shim_list_push 插桩链，b12 worktree 已撤销，证据链在本行）；**(c)** TodoList 子组件事件绑定 VM 模式全剥（D-GAP-4 家族，013 版实证：子树渲染但无任何 onclick 行）——handler_codegen 层，留档 | (a)(b) 待独立计划根治（ABI 手术）；(c) 留档 D-GAP-4 |
