...
| B12 | B10(b) 副产品 | 013-todo VM 模式三处 VM 语义缺口（2026-08-21 实机实证，desktop_mcp.py SKIP 注记即证据）：**(a)** store.Init 计数循环 `.todos[i].done` 对 to_array() 产 VmRef 数组元素取字段全非 false → `active_count=0`（种子 3 active）；**(b)** db 模块级全局（`var todos`/`var nextid=4`）在 Init 与后续 handler 之间被重置/复制——create_todo 读到 nextid=0/todos=[]，post-add list_todos() 只剩新条目或丢种子（两次运行 4→1 与 4→3 两形态，疑模块状态竞态/双实例）；**(c)** TodoList 子组件事件绑定 VM 模式全剥（D-GAP-4 家族，013 版实证：子树渲染但无任何 onclick 行）| VM 层调查（模块全局生命周期 + VmRef 元素字段访问），建议独立小计划或并入 242 tracker |
