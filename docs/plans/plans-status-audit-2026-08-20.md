...
| B12 | B10(b) 副产品 | 013-todo VM 缺口（2026-08-22 诊断改判 + 部分根治，`plan-fix/b12-abi`）：**(i) ✅ 幻影实参帧错位已修**——推翻此前"字符串2槽 ABI"理论（disasm 误读，字符串实为单槽）；真因：mcp type/submit 的 input_value 在 `on_with_input_for` 被无条件塞成 handler 实参，无参 handler（`onenter: .AddTodo`）即 n_args=2 vs prolog args=1 → 帧错位 → 状态写落垃圾对象 id。修复：VmBridge 记录 (widget,event)→声明参数数，门控 input_value（stash 实证红→绿：todos 6→5）；复现测试 ×3 锁定（plan370 真实 013 + payload 路径 + 合成守护）。**(ii) 残余：GUI 专属组件重建路径**——headless 生产路径与 payload 路径均已清白，但 GUI type+submit 仍现 todos=[probe(id:0)]；id=0+空 db = 重建 bridge 未重跑 `__module_init`（全局未初始化）铁证，疑 WriteState/type 触发 reload 未带 module_init——**下案入口**。**(iii)** 子组件事件剥离（D-GAP-4 家族）留档。回注：active_count 计数缺口(a)待(ii)修复后复测（可能与全局未初始化同源） | (i) 已关闭；(ii) 待下案（reload/module_init 生命周期）；(iii) 留档 |
