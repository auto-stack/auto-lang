# Plan 358 — Auto as Rust's Script Layer — Rollout Implementation Plan
...
git commit -m "fix(tests): make conformance_tests.rs header honest about scope (Plan 358 C1)
...
```
（已于 Plan 358 C1 修复：conformance_tests.rs 注释已诚实降级为"AutoVM 输出回归测试"。）
```
...
git commit -m "docs(strategy): note C1 conformance header fix completed (Plan 358)"
```
...
git commit -m "chore(parity): remove DBG355 debug residue from runner/main (Plan 358 C2)
...
git commit -m "test(parity): confirm p1/p2 green, record divergences (Plan 358 C2)"
```
...
git commit -m "ci: add parity p1/p2 gate on PRs touching a2r/auto-lang/parity (Plan 358 C2)"
```
...
git commit -m "feat(parity): add 'report' subcommand generating HTML dashboard (Plan 358 C3)
...
```bash
git add .github/workflows/parity-ci.yml
git commit -m "ci(parity): auto-generate and upload dashboard artifact (Plan 358 C3)"
```
...
git commit -m "test(parity): serde_json reaches L1 three-way green (Plan 358 D1)"
```
...
git commit -m "test(parity): regex reaches L1 three-way green (Plan 358 D1)"
```
...
git commit -m "feat(parity): add cli_app use case (wc-style, no-dep pure Rust output) (Plan 358 D1)"
```
...
git commit -m "docs(examples): add user-facing Script-to-Ship demos for D1 use cases (Plan 358 D1)"
```
...
git commit -m "feat(parity): add trait_advanced use case (default methods, assoc types, bounded generics) (Plan 358 D2)
...
git commit -m "feat(parity): add generators use case (yield iterators, lazy chains) (Plan 358 D2)"
```
...
```bash
git add examples/script-to-ship-demos/
git commit -m "docs(examples): add trait-demo and generators-demo Script-to-Ship demos (Plan 358 D2)"
```
...
git commit -m "feat(parity): add http_client_sync use case (synchronous ureq-based) (Plan 358 D3)"
```
...
git commit -m "test(parity): tokio reaches sorted-TAP parity for spawn/join + channel (Plan 358 D3)"
```
...
```bash
git add examples/script-to-ship-demos/
git commit -m "docs(examples): add http-client-demo and async-demo Script-to-Ship demos (Plan 358 D3)"
```
...
git commit -m "feat(website): scaffold ScriptShipView component (Plan 358 B1)"
```
...
```bash
git add website/.vitepress/theme/components/ScriptShipView.vue
git commit -m "feat(website): ScriptShipView VM-run + a2r-transpile logic (Plan 358 B1)"
```
...
git commit -m "feat(website): ScriptShipView side-by-side compare mode (Plan 358 B1)"
```
...
git commit -m "feat(website): register ScriptShipView global component (Plan 358 B1)"
```
...
git commit -m "feat(website): support <Listing view='scriptship'> -> ScriptShipView (Plan 358 B2)"
```
...
git commit -m "docs(script-to-ship): 6-chapter interactive tour (EN) (Plan 358 B2)"
```
...
git commit -m "docs(website): wire Script-to-Ship tour into sidebar (Plan 358 B2)"
```
...
git commit -m "docs(script-to-ship): Chinese translation of 6-chapter tour (Plan 358 B2)"
```
...
git commit -m "docs(website): v1 hero demo + narrative (L2 claims only) (Plan 358 A1)"
```
...
git commit -m "docs(website): three-act narrative + comparison table + status declaration (Plan 358 A1)"
```
...
```bash
git add website/index.md
git commit -m "docs(website): upgrade hero to L1-verified use case (Plan 358 A2)"
```
...
```bash
git add website/index.md
git commit -m "docs(website): link status declaration to live parity dashboard (Plan 358 A2)"
```
...
git commit -m "docs(website): bilingual landing + tour cross-link (Plan 358 A2)"
```
