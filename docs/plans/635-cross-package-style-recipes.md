...
1. **use 导入形态裁定（阻塞 T-02 开工，不阻塞 T-01/T-05/T-06）**：
   - 推荐：复用既有符号导入 `use common.styles: pill`（零新语法、与
     widget/fn/store 同构，本档 D1/T-02 按此书写）；
   - 备选：新语法 `use styles from common: pill`（显式区分样式符号，但引入
     新关键字与概念，无能力收益）。
2. **门控严格度**：D4 按「未声明=编译错误」硬门控设计；若 T-01 探针发现存量
   未声明使用面，逐条定性后裁定（修复存量 vs 降级警告 + 债项登记）。
3. **D6（pac.lock 补全）独立取舍**：轻量但与本计划核心目标正交，若执行期
   带宽紧张可单独摘出为 L0 任务，不影响 AC-01..05。
