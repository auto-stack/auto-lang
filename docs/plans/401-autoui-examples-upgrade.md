...
| 020-music-player | 已升级 | ✅ 完成 | [Plan 520](520-020-music-player.md) | Spotify 式迷你播放器(播放控制+搜索过滤+收藏持久化)，6/6 全绿 |
| 021-blog-viewer | 已升级 | ✅ 完成 | [Plan 521](521-021-blog-viewer.md) | Medium 式博客阅读器(列表+分类过滤+详情分段+点赞+发布表单+二次确认删除)，7/7 全绿 |
...
**推荐批次顺序**：022-kanban（✅）→ 023-realworld（✅ 阶段1+2）→ 024（✅ 经 409，旧目录已清理）→ 019（✅ Plan 519）→ 020（✅ Plan 520）→ 021（✅ Plan 521）。（025 已删除，不再排队；018-027 示例全线完成）
