...
| Jetpack Compose | `.kt` @Composable | Working |
...
- Add `toast` to `crates/auto-lang/src/ui_gen/jet/components.rs` mapping to `Snackbar` or `Toast` in Material3
...
- Verify `progress` output in `crates/auto-lang/src/ui_gen/rust.rs`
...
- Add avatar as a circular image container in `crates/auto-lang/src/ui_gen/rust.rs`
...
- `var email_error str = ""` and `var password_error str = ""`
...
6. `006-hero-section` — validates rich text + centering
7. `007-stats-board` — validates progress + data display
8. `008-pricing-table` — validates switch/toggle + conditional content
9. `009-article-feed` — validates avatar + repeated cards
10. `010-contact-form` — validates select/textarea + toast feedback
