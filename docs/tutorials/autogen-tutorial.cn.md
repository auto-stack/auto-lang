...
1. **数据段**：创建一个 Atom 数据结构（JSON 的超集）
   - 可以直接使用简单的 Atom 格式：`name: "World"`、`count: 42`
   - 可以使用数组：`items: [1, 2, 3]`
   - 可以使用对象：`user: { name: "John", age: 30 }`
   - 也可以使用带 `let` 的 Auto 脚本：`let name = "World"`（然后在 Atom 中引用为 `name`）

2. **模板段**：查询 Atom 数据并生成输出
   - 代码行（以 `$ ` 开头）：循环和条件等控制逻辑
   - 内容行：带有嵌入变量的模板输出

## 模板语法规则

在深入示例之前，了解 auto-gen 模板的工作原理很重要：

1. **代码行**（不产生输出）：以 `$ `（美元符号后跟空格）开头的行是 Auto 代码语句。这些行控制模板逻辑，但本身不产生输出。
   - 示例：`$ for item in items {` - 这是一个循环控制语句
   - 在这些行中，直接使用变量名：使用 `item` 而不是 `$item`

2. **内容行**（产生输出）：所有其他行都是模板内容，将进入最终输出。
   - 示例：`Hello, $name!` - 这一行产生带有嵌入变量的输出

3. **变量嵌入**：在内容行中，使用 `$` 来嵌入变量或表达式：
   - 简单变量：`$name` 或 `${name}`
   - 字段访问：`${user.name}`
   - 复杂表达式：`${items.length()}`
...
分解说明：
- 第 1 行：`$ for ...` - 代码行（以 `$ ` 开头），不产生输出
- 第 2 行：`用户：${user.name}` - 内容行，产生带有嵌入字段访问的输出
- 第 3 行：`$ }` - 代码行，关闭 for 循环
