...
2. **参考实现集(`.at`)** —— 见 §2.4:不止一份,而是**每个 variant 一份**,作为默认拷贝源 + 回归 fixture + 变体范围的物化样本。

于是**两者都有**:要开箱即用的标准 LoginBox → 拷某个 reference;要适配你的 SSO+2FA+品牌 → 让 AI 照 spec 生成。维护 reference 反过来校验 spec。这也呼应 `/auto-lang-creator` skill —— **block spec 本质是一份"经库策展的、专门的 skill 文件"**。

### 2.4 两个一等模块:参考实现集 + gotchas

spec/文字描述"如何组装",但**具体代码对 AI/人都远胜文字**。故 block 定义除 spec 外,还应有两个一等模块:

**(a) 参考实现集(reference implementations)** —— 不止一份,而是**每个 variant 一份**(minimal / with_sso / magic_link …)。价值三重:
- **默认拷贝源**(要"标准版"者直接拷)。
- **回归 fixture**("AI 能否仅凭 spec 复现等价物?"——既做 spec 仓 CI,也做 Design 16 的 AI 修复轮次 N 的评测锚)。
- **变体范围的物化样本**——把 spec 说的"本 block 吸收这些变化"用真代码落到底,展示 block 的柔性边界。

**(b) Gotchas(反例模块)** —— 负样本是强有力的教学。每条结构:**错误做法 + 为什么错 + 正确做法**(尽量带反例代码)。比 prose 的 pitfalls 列表更具操作性。Gotchas 可**从稀疏起步**,随 AI 实际失败模式累积(AI 每生成该 block 犯一次错 → 沉淀一条 gotcha)—— 自增强闭环。

三者合成一个 **block 包**(类 skill 库的组织):

```
blocks/form/login/
  spec.md            # frontmatter + intent + 组装指引
  reference/
    minimal.at       # 每个 variant 一份参考实现
    with_sso.at
    magic_link.at
  gotchas.md         # {错误, 为什么, 正确}(带反例代码)
```

参考实现同时是 spec 自身的 CI fixture,故非额外负担,而是本就该有的测试床。
...
# References(见 reference/*.at,每 variant 一份)
minimal / with_sso / magic_link

# Gotchas(见 gotchas.md,每条 = 错误 + 为什么 + 正确)
端点写死 / 漏 loading-error 契约 / label 未关联 / ……
```
...
- **Phase A(block 包基础)**:定 **block 包**格式(spec frontmatter + NL 正文 + acceptance + **参考实现集 reference/*.at** + **gotchas.md**)+ `BlockRegistry` 数据模型 + `examples/blocks-gallery` 骨架(先 Form + Data-display 两类)。每类手写 1-2 个 block 包:**spec + 每个 variant 一份参考实现 + 起步 gotchas**。参考实现同时作 spec 的回归 fixture(能否 AI 复现)。产出:包格式规范 + 两类各 1-2 个完整 block 包。
