这是一个基于我们所有深度讨论（AIE 架构、Task+Msg 异步模型、统一 FFI 语法）汇总而成的 **AutoVM (AutoVM Edition)** 最终设计蓝图。

这份文档将作为 AutoVM 开发的权威指导。

---

# AutoVM (AutoVM) High-Level Design Document
...
**AutoVM** 是 Auto 语言在 PC 端的参考实现。它不仅仅是一个解释器，更是一个 **"Polyglot Concurrency Runtime" (多语言并发运行时)**。
...
AutoVM 采用 **"Hub-and-Spoke" (轮毂)** 架构，核心是异步的执行引擎，外围是动态插件系统。
...
AutoVM 采用 **"Virtual Stackful Tasks on Green Threads"** 模型。
...
这是 AutoVM 的核心竞争力。通过统一的语法和插件机制，打通异构语言壁垒。
...
AutoVM 作为 AIE 的客户端，必须支持 **Hot Patching**。
...
为了保证代码在 MCU 上能跑，AutoVM 必须“假装”自己很穷。
...
* **对架构**：AutoVM 既是 MicroVM 的仿真器，又是强大的 PC 端胶水运行时。
