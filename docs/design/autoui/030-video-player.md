...
**Vue 链的真实播放也已收口**（T-03/T-04/T-07/T-08，e2e 13/13 绿；T-09..T-14 于
2026-09-13 收尾批落地）。仍未接上、且**刻意不接**的是「默认可用」——native 播放是
显式开启的 feature（`--features mpv`），默认档走诚实降级面板（AC-20 的门控裁定）。

### 4.14 决策件收编（PLAN-617 T-13）

本节把散在 §4.1–§4.12 的结论按「决策件」形态收拢，作为后续播放能力立项的权威引用：

- **推荐路径**：§4.1 四选一裁定（SW 采用 / GL、sidecar、FFmpeg-FFI 排除），支撑数字
  在 §4.2（4K 端到端 7.3 ms/帧、实时时钟 0.997×；1080p 258–385 fps 上屏通道实测）。
- **许可与分发**：§4.8——libmpv LGPLv2.1+、本计划不分发 DLL、运行库解析序与降级
  行为已入规范（SD-05）。
- **硬约束**：§4.5——帧上屏必须绕开 iced 图像通道（每帧新建 `Handle` 的新 id  mint
  与纹理重上传即 `ui/iced/renderer.rs:2889-2898` 记录的闪烁机理）；纹理建一次。
- **与 `imagesurface`/`ImageSurface` 的接入点**：视频**不**复用 `image_pipeline` 的
  票据/rendition 管线（那是静态图片的抽帧/缩放缓存形状，视频帧流是持续最新帧语义，
  由 `ui/mpv/channel.rs` 的 latest-wins 代际门承担）。未来接入点有二：① 若做
  **视频缩略图**（PLAN-617 非目标 N6），正确形状是后端抽帧后走既有
  `image_pipeline` 的票据通道，而不是让缩略图走 mpv 实时通道；② `View::Video` 与
  `View::ImageSurface` 同形（均不携带消息），后续若把渲染面统一，二者的 widget
  契约是现成的对齐面。
- **状态**：本决策件对应的实现已交付并实机验证（§4.9–§4.12）；VM 端 `video` 支持
  级别为 `partial`（`schema/aura.at`），默认档可降级。
