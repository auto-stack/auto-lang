...
`map_async` + `unmap` 一次往返 **0.11–0.14 ms**（环形缓冲换页的固定成本，可被 3 槽环掩盖）。
...
| 风险 | 实测依据 | T-17 的处置 |
|---|---|---|
| 通道 C 有**极稀有的长尾**（4K 120 帧里 1 帧 957 ms、1080p 里 3 帧 242 ms） | 门控 B 离群计数 | 用 **2–3 槽 staging 环**（避开「映射中的缓冲不可提交」）+ 单帧丢弃策略；长尾定位在 T-17 的第一件事 |
