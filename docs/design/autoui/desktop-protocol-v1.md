...
| **帧载荷 v2（PLAN-683 SD-01）** | `DisplayList`（载荷种类 tag **2**，v1 DrawList=1 冻结共存——宿主按 shm 槽内首字节分派，互斥不静默降级）：v1 原语超集——Quad 全参（Fill=色/线性渐变 **angle+stops 原生**〔与 iced 同构角度模型，start/end 由 rect 中心派生〕/四角半径/BorderSpec/ShadowSpec）+ `Transform{matrix:2×3}` 压出栈 + TextStyled/Scissor/Image 同形；`from_v1` 直搬映射（legacy 生产者零改造）。信封：`FrameReadyV2`（tag 12 追加，内嵌形态）；shm 形态沿用 `FrameReadyShared` 元数据（槽内 tag 区分）。生产者 = headless iced 宿主（`iced::Renderer::Secondary(tiny_skia)` 记录层截获——remote 模式，RqProjector 不再产 v2） | `message.rs` / `headless.rs` / `broker_surface.rs` |
| 三态渲染开关（+remote 四值，PLAN-683 SD-02） | per-App：`desktop_render: auto \| queue \| independent \| remote`——pac.at manifest 字段（Plan 276 既有前端后端 `render:` 字段撞名，语义正交不可复用，执行定案改名）+ spawn 参数 `--autodesk-render=<mode>` 覆盖（CLI `run` 具名 `--render` 撞名）+ 裁决链（spawn 参数 > manifest > auto；进程形态 `adjudicate()` 链正交）。`auto` = 装载期覆盖度探测，可行走 queue、不可行降级 independent（宿主记观测 `Log` 一行）；`remote`（PLAN-683）= headless iced 宿主 + DisplayList v2 产线帧（**覆盖门不适用：组件覆盖 = iced 全集，构造保证**——coverage 概念对该模式失效），CLI 侧经 `AUTO_VM_RENDER` env 分岔 | `cmd_autodesk.rs` / `coverage.rs` / `auto-man pac.rs` / `headless.rs` |
