...
**日期**: 2026-07-17
**状态**: Accepted（母纲已确认；配套实施计划见 `docs/plans/359-auto-as-rust-script-rollout.md`）
**作者**: ZCode brainstorming session
**性质**: 高层策略纲领（high-level strategy framework）。本文档是 4 个子项目（A 门面 / B 旗舰 / C 地基 / D 弹药）的**母纲**。配套实施计划 `docs/plans/359-auto-as-rust-script-rollout.md` 将母纲拆解为可执行的阶段任务；后续每个子项目仍可各自出更细的 spec。
...
> ✅ （已于 Plan 359 C1 修复：`conformance_tests.rs` 注释已诚实降级为"AutoVM 输出回归测试"，明确指向 `parity/` 负责真正的 VM↔a2r 一致性验证。）
...
**关键决策点（留待 D 子项目 spec）**：
- 用例库放哪？建议 `parity/libs/<category>/<name>/`（Plan 458 分类布局，与现有 parity 框架一致）+ 在 `examples/` 下做面向用户的副本。
...
本纲领已确认。配套实施计划：**`docs/plans/359-auto-as-rust-script-rollout.md`**。

该计划按 C → D → B → A 的推荐顺序，把母纲的 4 个子项目拆解为分阶段的可执行任务（带文件路径、代码、命令、出口条件、提交点）。

子项目之间的关系（地基 → 弹药 → 旗舰 → 门面）决定了它们必须按序推进，但 A 的最小版可与 C 并行探路。计划中每个阶段都有明确的"出口条件"和"解锁项"，便于阶段性验收。

后续若某个子项目（尤其 C 和 D）需要更细的专项 spec，可再单独立项，以本母纲为依据。
