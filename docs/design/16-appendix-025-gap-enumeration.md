...
## this.n 修复(2026-07-02)

`parse_event_arg` 把独立 `.field` 编为 `this.field`(ArkTS 正确,Vue 错误)。Vue `<script setup>` 用裸 state ref,故 `VueGenerator::handler_to_function_call_with_params` 现对每个 event 实参 strip 前导 `this.`。生成 `Bump(n + 1)` 而非 `Bump(this.n + 1)`。仅 Vue 侧,ArkTS 不受影响。K2 canary 现用计算型实参 `.Bump(.n + 1)` 全绿。

## K3 移除(2026-07-02):rich-text 非 auto-lang 职责,归 AutoDown

原 K3"在 auto-lang Vue codegen 里渲染 markdown"经复核**架构错位**:rich-text(markdown 超集渲染+编辑)是独立工程 `../auto-down` 的职责,提供 `@autodown/vue`(renderer)+ `@autodown/editor`(Tiptap)+ `core`。AutoUI 应用经**手写 wrapper 组件**消费这些包(jade-garden 已验证:`EditorTab.vue` import `AutoDownEditor`)。auto-lang 再发明一份 markdown 渲染 = 重复。

故 K3 从 Plan 345 移除。AutoUI 富文本走"消费 @autodown 包"的跨工程集成(app 层决策),非 auto-lang codegen 缺口。**Plan 345 至此完成**(N1/K2·N4/N2/N3/OOM/this.n 全闭环,K3 移除)。

## handler 参数名一致性(2026-07-02)

for-loop 内带显式实参的事件 handler(如 `.SelectNote(note.id)`)被错误注册为 loop-param handler,导致生成函数签名参数名 = 循环变量(`note`),而 handler body 用声明参数名(`id`)→ vue-tsc "Cannot find name 'id'"。根因:`loop_param_handlers.insert` 在 `params.is_empty()` 判断**之外**(三处)。修复:把 insert 移进 `is_empty()` 块 —— 只有真正自动补循环变量的 handler(空实参)才注册。025 SelectNote 现生成 `function SelectNote(id: any)` 与 body 一致,vue-tsc 绿。
