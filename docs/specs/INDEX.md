...
| 模块 | 职责 | 状态 |
|---|---|---|
| frontend | lexer/token/parser/AST/dialect/resolver/宏 | implemented（核心管线完整；符号属性/位操作/Auto Flow 未实现） |
| types | 类型推断 infer/、typeck、ownership（borrow/lifetime/cfa）、trait_checker | implemented（核心可用；借检查、参数检查集成仍 partial） |
| comptime | 编译期求值 | partial（语句级 `#if/#for/#is/#{}` 已实现；表达式级编译期替换/沙箱/限额未落地） |
| interpreter | AutoVM 编程式求值外观（旧 TreeWalker 已删，plan-091） | partial（`call()` 未实现、结果提取不完整） |
...
| widgets | active | 6 | [widgets/project.md](widgets/project.md) |
...
| Project | 状态 | 模块数 | 项目卡 |
|---|---|---|---|
| parity | active | 9 | [parity/project.md](parity/project.md) |
