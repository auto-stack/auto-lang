...
| 615 | calc-ux-fix-and-enhance | ✅（reviewed→archived） | archive/615-calc-ux-fix-and-enhance.md | VM `&&`/`\|\|` 短路求值根修（急切 AND/OR 单指令→DUP+JMP_IF_Z/NZ+PUSH_BOOL 占位+末端归一双路径栈平衡发射；修复 calc 011 Equals 空栈 ops[-1] IndexError 守卫崩溃〔Plan 550 翻转引爆〕，跨端佐证 TS/Py/C/Rust/GD 全短路）+语料 99_short_circuit 三件常驻+SD-01 落 vm/overview §布尔短路求值语义；兼容扫描存量 10 处 && 全纯布尔零急切依赖；tv 3679/3679；账本 P615-D3/D4 |
| 648 | run-gate-codegen-defects | ✅（reviewed→archived） | archive/ | T-10 dev 跑法 api 委托断供根修：split 模式 api-over-HTTP 改写从裸名-only 扩至模块别名限定调用（`api.get_lines()`，Expr::Dot(Ident,fn)）——两合成点 api_funcs 增限定名键（扁平化全名+别名末段.fn）、codegen 门卫放行；此前限定调用静默回落 back 链进程内执行（宿主引擎 DLL 缺席降级→全空，auto-term 空屏定因）；021 "第三断点"证伪+AUTO_FFI_TRACE 归因 auto-term 侧；AUTO_LANG_HTTP_TRACE=1 仪器常驻（simple_http_json 汇聚点）；次生发现 P648-D1 handler 再入缺陷（KNOWN-DEBT 三次实证）；实机 1063/4083×200+Tab"● shell 1"渲染；tf/tv/vm_file/plan340 零新增红 |
| 658 | uigallery-multi-backend-proxy | ✅（reviewed→archived） | archive/658-uigallery-multi-backend-proxy.md | 画廊内嵌 fullstack demo 后端半身：单进程多后端宿主 back-proxy——每 app back 链独立 VM session（专线程+mpsc，api_routes 自持绕过全局单表）+ 子 URL /apps/<id>/ 路由（#[api] JSON 按名绑定/~Stream 签名特路 SSE+api_gen 同款 POST 广播/原生 media scan-stream/图片字节委托）+ 崩溃隔离（catch_unwind+指数退避重装载+日志环+panic_hard 测试面）+ 发射器解否决（client 模块+Tick SSE 消费+字面量前缀化+fixtures 锚定）；AC-01..05 全实证（020 393 曲库/017 双向+Typing≤400ms/031 双文件字节 MATCH/单端口同 PID/panic 隔离重启）；spec vm/back-proxy.md 新建 |
| 659 | crash-forensics-verify-infra | ✅（reviewed→archived） | archive/659-crash-forensics-verify-infra.md | P642 稳定性/验证基建债承接：F1 崩溃族三场 cdb 托管 soak 三族归因（wgpu offscreen 零维 panic 根修三路截图守卫 / virt_memory 堆 id 误作栈地址定罪方向 P659-D1 / 原生本体 17 轮零复现护栏在位）+ min>max dash clamp 上界守卫 + MCP fixture trigger 双形态（裸名跨名空间解析+未命中响亮报错）+ fs.canonical 失败返空哨兵（vm/native-fs.md spec 新建）+ 020 folder-music→lucide 0.312 有效名（vue build 绿+web 挂载可视）+ flaky 三成员定罪（双根修+display_family 特性面差异归因修正）+ 022 门控债清偿；判读法/soak 编排器常驻 auto-os ui-gallery tests；账本 P-659-1 |
| 662 | gallery-fast-start-and-viewport | 🚧 executing（work 收口中） | plans/662-gallery-fast-start-and-viewport.md | 画廊启动性能与视口滚动：实测瓶颈=逐 demo from_workspace 扫描（77s/36 串行；demo 合并编译仅 3s——懒加载证伪非靶，记远期）→ 三层修——①gallery_apps_dir 补祖父级 auto-lang 探测（平铺主检出/worktree 组同形，主检出一键裸跑解锁）②行扫描磁盘缓存 gallery_cache.rs（per-demo FNV 内容哈希[仅文件行——目录 mtime NTFS 延迟落定实测漂移]+~/.auto/auto-man/gallery-cache env 覆写+fail-open+原子写+registry 字节一致守卫）③未命中扫描 std::thread::scope 并行（thread_local 注册表隔离+产物字节 diff 全等→默认核数；AUTO_GALLERY_SCAN_JOBS 覆写）+AppViewport.vm.at frame 内滚动容器对齐 web 臂（020 底部裁切根修，T-05 裁定语料零改动[flex-wrap 登记 P662-D1]）；实测冷启 83→22s（并行）/二次 83→5s；SD-01 ui/overview §PLAN-662 六条 |
| 667 | memory-safety-baseline | ✅（reviewed→archived） | archive/667-memory-safety-baseline.md | 现有内存安全底线：F-01 帧身份守卫（(task_id,frame_uid) 活跃表，by-ref 捕获读写前拒绝已返回帧/同 bp 复用/跨任务；嵌套闭包传递继承祖父帧锚；STORE_CAPTURED 双 pop/影子转移四同族缺陷齐修）+F-02 rc_stats 纯化（显式 reap_all 收尾分离）+F-07 RC 三重门（存在性门/防御补持收紧 tagged/份额出处门——真整数撞活 id 零冒领）+F-03/04 a2r 闭包捕获真实下降+fold_to_root 名称合并+fail-closed+F-05 `.mut`/Send 边界明确诊断；21 探针三矩阵全绿+rustc 实编正例；SD-01..04（新增 vm/design/memory-safety-boundary.md 边界矩阵）；债 P667-D1/D2；账本 P667-1/2 |
| 669 | vm-server-api-arg-binding | ✅（reviewed→archived） | archive/669-vm-server-api-arg-binding.md | VM 轨 #[api] 实参装配断层修复（auto-edit 勘定转交，源报告定性反转成立）：API_PARAM_SIGS codegen 侧信道+共享绑定器 bind_api_args_by_name（path→body 字段→query 按名/缺参·类型错 400 指名/元数据按名 opt-in=meta 族/单参 whole-body 容忍/无签名位序回退）+四装配点收敛（async 生产+native listen+两死码统一 bind_api_args_or_legacy，find_route 死码移除）+**入口分片根修**（async 请求头单次 read() 遇 TCP 分片=plan 568 T7 th 档"环境红"真身，读至 \r\n\r\n 后 master 28 预存红转绿）；spec stdlib/design/http-server.md §4.1 五条注入规则+§4.1.1 实现状态；复审两轮（R1 回退臂单测/R2 同步路径元数据差异句/R3 注释）；账本 P669-1/2 |
| 700 | unify-vm-files-tier | ✅（reviewed→archived） | archive/700-unify-vm-files-tier.md | test-vm-files 退役+语料三族（162 测）常驻化：tv 降 profile filter 档（[profile.tv] 白名单+--profile tv；-E 内联 filterset 于 Windows cargo alias 不剥引号不可行，单/双引号皆败实测）+tf 全档含语料+t 镜像 churn 排除——t→tf→tv→t 四段零 Compiling 同二进制零重编实证（tv 162/162 绿 3.95s、tf 5689 含语料零红、红册 12 全预存：plan606 test_029 master 复现定责+ffi_dual_019 环境轮换）；R-1 死常量 COOKBOOK_DIR 摘除；aavm 独立空 feature+scripts×3/CI/AGENTS/project.md 口径同步；观察=gallery 围栏每进程全量重编 ~800s 无跨进程缓存（Plan 700） |
| 701 | edit-consume-supply-pack（供④ time 族） | ✅（reviewed→archived） | archive/701-edit-consume-supply-pack.md | time 族 1200/1201/1205 VM 运行时 shim 实装（此前裸/use 均返 0）：Unix epoch 三方一致（.at 声明/time.rs.at/a2r-std 对拍<5s；计划「单调钟」措辞勘正=立项笔误）+Instant.elapsed 约定 Void→I64 毫秒（.at None→值形）+vm_bridge nv_to_pub_value 补 TAG_I64 臂（桥面垃圾值实证修）；.at 层 TAG_I64 算术解码错位=预存平台缺口 P701-D1（#[ignore] 留档）；SD-02 time-natives.md（Plan 701） |
