...
- 决策：`AutoVM` 新增段执行驱动 `call_fn_by_name_segment` / `resume_fn_by_name_segment`（共享核 `drive_handler_segment`，`allow_busy_wait` 开关）：Yield 且 `waiting_http_request_id`/`waiting_future_id` 就位时**不进入忙等**，返回 `SegmentOutcome::Parked{wait, seg}`（task 持 ip/bp/栈/闭包上下文天然可续）；恢复入口镜像 wake source 6（外部 future 推结果）+ `resume_suspended_body`（async_frames 续体）后续跑同一 step 核。**PLAN-027 缺陷 A 边界**：`drop_async_result` 回收只属"真正放弃等待"的超时路径，parked 不回收、恢复时 shim 重入臂按 req_id 消费。**忙等语义退役为 legacy**：`call_fn_by_name` 原样保留给自带线程阻塞可接受的非 UI 调用方（back_proxy / http_server / Rust 侧原生宿主等）；UI 桥（vm_bridge）handler 派发族全迁移段驱动（含 run_module_init——ui/architecture.md ADR-24），仅 view 侧求值两处带标记保留（ui/ 下门禁测试封顶）。等待上限契约变更：从"30s 忙等硬超时→RuntimeError"变为"reqwest 客户端超时以错误体数据形态落账（.at 可捕获）"。
