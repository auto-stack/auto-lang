...
| 681 | a2r-deploy-gap-batch（trans 侧） | ✅（reviewed→archived） | archive/681-a2r-deploy-gap-batch.md | 内建映射面契约成文（overview 新节）：Env→env 归一化+fs.tree 双表+宿主位两类（a2r-std 独立 crate 经迷你库 re-export 单源）+动态记录面（bare List→Vec<Value>：Value 派生推断/字段索引访问/push json!/typed 收口/形参收口——窄门仅 Value 语境）+fs.tree 双轨逐字节对拍（fs_tree_parity 先例）；tt 4091/4091 |