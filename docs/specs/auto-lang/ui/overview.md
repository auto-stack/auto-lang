...
直译〔I6 与 layout.rs 共享期望值表 layout_cases.json——表内 `reservedTaskbar` 与 layout.rs `TASKBAR_HEIGHT` 须双端同值（56，PLAN-526 T24 起；Plan 686 勘正 TS 侧残留 48 与 Rust 测试期望漏同步）〕、keyboard.ts R12 热键捕获段、
