...
| 602 | py-subclass-class-factory（frontend 侧 a2py 轨） | ✅（reviewed→archived） | archive/602-py-subclass-class-factory.md | `py_subclass` 发射臂 → `_auto_subclass` helper 注入（needs_subclass_helper 旗标族）：exec+setattr 同构工厂，Str 值缩进归一与 VM 臂同律（非空行统一+4）、callable 值经对象字面量 lambda 发射 setattr 为真函数（self 绑定与 VM def 包装器对齐）；探针双轨 stdout 一致 + p12 三轨 4/4；trans::python::tests 99/99 |
| 545 | use-namespace-semantics | ✅（reviewed→archived） | archive/545-use-namespace-semantics.md | bare `use db`=命名空间（Linker dep 限定注册 `db#sym`+点分别名/入口独占裸名 add_entry_module/删 strip-prefix 平铺后门；TypeStore bare 不 merge+modules 注册表）；`use db: *` 显式平铺+快照式冲突检测（pre-parse 快照规避 parser 写污染，同名异源异定义报错）；codegen wildcard→import_scope 映射；native/widget 注册去 bare 通配；a2r bare 发射 `use crate::X;` 无 glob；R1 独立探针补证类型/枚举等价面+db.Note 语法层预存拒绝定案（文档措辞对齐）；use_semantics 7 测 |
