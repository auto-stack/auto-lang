# Auto Language Conformance Specification

Auto 的两个执行后端（AutoVM 和 a2r）必须对同一份代码产生完全一致的运行结果。本目录定义两者的**语义一致性标准 (Conformance Specification)**。
...
- **Layer 2 对偶测试** (`test/a2r/conformance/`) 验证一致性规范与实现一致
- **Layer 4 差分测试** (`test_util::program_generator`) 随机生成程序，验证 AutoVM 不 crash
- 发现不一致时以测试结果为准，更新一致性规范
- 新特性开发流程：先写一致性规范 → 对偶测试 → 实现

## Conformance-Driven Development 流程 (Phase 5)

新语言特性必须按以下顺序开发：

```
1. 编写一致性规范 (docs/conformance/XX-feature.md)
