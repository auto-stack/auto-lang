...
结果（2026-09-25 11:02-12:34 实测）：

| run | 结果 | 早崩 | 测试级失败 |
|---|---|---|---|
| 1 | 完成 | ❌ | 9（T13.6/13.7/T16.4/T17.x——拥堵期串话面） |
| 2 | 完成 | ❌ | 7 |
| 3 | 完成 | ❌ | 19（拥堵峰值） |
| 4 | 完成 | ❌ | 6 |
| 5 | 完成 | ❌ | 13 |

**早崩清零 5/5 达成**（基线旧工具链 2/5）。测试级失败集漂移（6-19）
与本勘定的并行会话拥堵谱一致（watcher 实录：11:03:00 同秒六进程
生灭、他会话抢占 9249/9250 端口带）——非工具链面，判据「早崩清零非
全绿」不受扰。注：run4/5 exe 含 javascript.rs print 改道（后续
c9901f762 回退——JS golden 字面断言，VM 矩阵零涉，谱有效性不受
影响）。下游 T-09 口径评估可直接引用本谱+§3 根因定罪。
