...
| 438 | 025-dashboard 系统监视器 | ✅ | M1 vue 三区布局（KPI 卡+三面积曲线+可排序进程表，浏览器实机 6/6）+ M1-fix ts_adapter 双缺口根治（FStr 臂 .value 解包/int 局部 Math.trunc 追踪）+ M2 vm 模式（table kebab 合一/bindings 提取/badge text prop 三修 + storage 文件背书，desktop_mcp 26/26 含重启恢复闭环）+ §9.1 复审收口（侧栏降级决议/439 需求单/Tick interval 债务登记） |
...
## Status Summary
- Completed: 40 | Partial: 4 | Planned: 11 | Deprecated: 0
