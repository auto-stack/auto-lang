...
| 410 | check_symbol Error Span | ✅ | undefined_variable 报错 span 从解析器游标改为表达式起始标识符（check_symbol err_span 参数 + 3 调用点 + error_spans_tests；Phase 2 系统性表达式 span 留后续，Expr::Dot 不查符号登记债务簿） |

Status codes: Completed, Planned, Partial/In Progress, Deprecated

## Status Summary
- Completed: 5 | Partial: 1 | Planned: 0 | Deprecated: 0
...
- Plan 410 Phase 2（系统性表达式 span，触发条件见归档计划 §4）
