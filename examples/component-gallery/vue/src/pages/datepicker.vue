<!-- DatepickerPage component - Auto-generated from Auto language -->
...
const showDatepickerBasicCode = ref<boolean>(true)
const activeDatepickerBasicTab = ref<string>('auto')
...
const datepickerBasicAutoCode = `popover {
    popover-trigger {
        button (text: "Pick a date", variant: "outline") {}
    }
    popover-content {
        calendar {}
    }
...
`
const datepickerBasicVueCode = `<Popover>
  <PopoverTrigger>
...
`
const codeblock1Code = `npx shadcn-vue@latest add date-picker`
watch(activeDatepickerBasicTab, () => {
...
<template>
  <div class="flex flex-col">
    <div class="flex flex-col gap-4">
      <h1 class="text-4xl font-bold tracking-tight">DatePicker</h1>
      A date picker component with a calendar popover for selecting dates.
      ...
      <!-- Merged DatepickerBasic Component -->
      <div class="rounded-lg border overflow-hidden">
        ...
        <div class="border-t">
          <button
            @click="showDatepickerBasicCode = !showDatepickerBasicCode"
          ...
          >
            ...
            <svg
              :class="showDatepickerBasicCode ? 'rotate-180' : ''"
          ...
          <div v-if="showDatepickerBasicCode" class="border-t">
            <!-- Tabs (gray title bar) -->
            <div class="flex items-center justify-between bg-muted">
              <div class="flex">
                <button
                  @click="activeDatepickerBasicTab = 'auto'"
                  :class="activeDatepickerBasicTab === 'auto' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Auto
                </button>
                <button
                  @click="activeDatepickerBasicTab = 'vue'"
                  :class="activeDatepickerBasicTab === 'vue' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Vue
                </button>
              ...
              <button
                @click="copyCode(activeDatepickerBasicTab === 'auto' ? datepickerBasicAutoCode : datepickerBasicVueCode, 'datepicker-basic')"
              ...
              >
                <svg v-if="copiedCode !== 'datepicker-basic'" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>
                {{ copiedCode === 'datepicker-basic' ? 'Copied!' : 'Copy' }}
              </button>
            </div>
            <!-- Code content with syntax highlighting -->
            <pre class="overflow-x-auto p-4 text-sm bg-[hsl(var(--code-bg))] text-[hsl(var(--code-fg))]"><code :class="'block font-mono !p-0 language-' + (activeDatepickerBasicTab === 'auto' ? 'auto' : 'html')">{{ activeDatepickerBasicTab === 'auto' ? datepickerBasicAutoCode : datepickerBasicVueCode }}</code></pre>
          </div>
        </div>
      </div>
      ...
      <Table>
        ...
        <TableBody>
          ...
          <TableRow>
            ...
            <TableCell>-</TableCell>
            <TableCell>Date display format</TableCell>
          </TableRow>
          <TableRow>
            <TableCell>placeholder</TableCell>
            <TableCell>string</TableCell>
            <TableCell>"Pick a date"</TableCell>
            <TableCell>-</TableCell>
          ...
          <TableRow>
            ...
            <TableCell>Disables the picker</TableCell>
          </TableRow>
        </TableBody>
      </Table>
    </div>
