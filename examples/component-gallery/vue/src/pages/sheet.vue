<!-- SheetPage component - Auto-generated from Auto language -->
...
const showSheetBasicCode = ref<boolean>(true)
const activeSheetBasicTab = ref<string>('auto')
...
const sheetBasicAutoCode = `row (gap: "4") {
    sheet (side: "right") {
        sheet-trigger {
...
`
const sheetBasicVueCode = `<div class="flex flex-row gap-4">
  <Sheet>
    <SheetTrigger>
...
`
const codeblock1Code = `npx shadcn-vue@latest add sheet`
watch(activeSheetBasicTab, () => {
  ...
  <div class="flex flex-col">
    <div class="flex flex-col gap-4">
      ...
      <div class="rounded-lg border overflow-hidden">
        ...
        <div class="border-t">
          <button
            @click="showSheetBasicCode = !showSheetBasicCode"
          ...
          >
            ...
            <svg
              :class="showSheetBasicCode ? 'rotate-180' : ''"
          ...
          <div v-if="showSheetBasicCode" class="border-t">
            <!-- Tabs (gray title bar) -->
            <div class="flex items-center justify-between bg-muted">
              <div class="flex">
                <button
                  @click="activeSheetBasicTab = 'auto'"
                  :class="activeSheetBasicTab === 'auto' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Auto
                </button>
                <button
                  @click="activeSheetBasicTab = 'vue'"
                  :class="activeSheetBasicTab === 'vue' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Vue
                </button>
              ...
              <button
                @click="copyCode(activeSheetBasicTab === 'auto' ? sheetBasicAutoCode : sheetBasicVueCode, 'sheet-basic')"
              ...
              >
                <svg v-if="copiedCode !== 'sheet-basic'" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>
                {{ copiedCode === 'sheet-basic' ? 'Copied!' : 'Copy' }}
              </button>
            </div>
            <!-- Code content with syntax highlighting -->
            <pre class="overflow-x-auto p-4 text-sm bg-[hsl(var(--code-bg))] text-[hsl(var(--code-fg))]"><code :class="'block font-mono !p-0 language-' + (activeSheetBasicTab === 'auto' ? 'auto' : 'html')">{{ activeSheetBasicTab === 'auto' ? sheetBasicAutoCode : sheetBasicVueCode }}</code></pre>
          </div>
        </div>
      </div>
      ...
      <Table>
        ...
        <TableBody>
          <TableRow>
            ...
            <TableCell>Which side the sheet slides from</TableCell>
          </TableRow>
          <TableRow>
            ...
            <TableCell>Controls the open state</TableCell>
          </TableRow>
        </TableBody>
      </Table>
    </div>
