<!-- ProgressPage component - Auto-generated from Auto language -->
...
const showProgressBasicCode = ref<boolean>(true)
const activeProgressBasicTab = ref<string>('auto')
...
const progressBasicAutoCode = `progress (value: "33", class: "w-60") {}
`
const progressBasicVueCode = `<Progress :model-value="33" />
`
const codeblock1Code = `npx shadcn-vue@latest add progress`
watch(activeProgressBasicTab, () => {
...
<template>
  <div class="flex flex-col">
    <div class="flex flex-col gap-4">
      ...
      <!-- Merged ProgressBasic Component -->
      <div class="rounded-lg border overflow-hidden">
        ...
        <div class="border-t">
          <button
            @click="showProgressBasicCode = !showProgressBasicCode"
          ...
          >
            ...
            <svg
              :class="showProgressBasicCode ? 'rotate-180' : ''"
          ...
          <div v-if="showProgressBasicCode" class="border-t">
            <!-- Tabs (gray title bar) -->
            <div class="flex items-center justify-between bg-muted">
              <div class="flex">
                <button
                  @click="activeProgressBasicTab = 'auto'"
                  :class="activeProgressBasicTab === 'auto' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Auto
                </button>
                <button
                  @click="activeProgressBasicTab = 'vue'"
                  :class="activeProgressBasicTab === 'vue' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Vue
                </button>
              ...
              <button
                @click="copyCode(activeProgressBasicTab === 'auto' ? progressBasicAutoCode : progressBasicVueCode, 'progress-basic')"
              ...
              >
                <svg v-if="copiedCode !== 'progress-basic'" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>
                {{ copiedCode === 'progress-basic' ? 'Copied!' : 'Copy' }}
              </button>
            </div>
            <!-- Code content with syntax highlighting -->
            <pre class="overflow-x-auto p-4 text-sm bg-[hsl(var(--code-bg))] text-[hsl(var(--code-fg))]"><code :class="'block font-mono !p-0 language-' + (activeProgressBasicTab === 'auto' ? 'auto' : 'html')">{{ activeProgressBasicTab === 'auto' ? progressBasicAutoCode : progressBasicVueCode }}</code></pre>
          </div>
        </div>
      </div>
