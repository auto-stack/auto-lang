<!-- NavigationmenuPage component - Auto-generated from Auto language -->
...
const showNavigationmenuBasicCode = ref<boolean>(true)
const activeNavigationmenuBasicTab = ref<string>('auto')
...
const navigationmenuBasicAutoCode = `row (gap: "4") {
    navigation-menu {}
}
`
...
`
const codeblock1Code = `npx shadcn-vue@latest add navigation-menu`
watch(activeNavigationmenuBasicTab, () => {
...
<template>
  <div class="flex flex-col">
    <div class="flex flex-col gap-4">
      <h1 class="text-4xl font-bold tracking-tight">NavigationMenu</h1>
      A collection of navigation links with dropdown support.
      ...
      <!-- Merged NavigationmenuBasic Component -->
      <div class="rounded-lg border overflow-hidden">
        ...
        <div class="border-t">
          <button
            @click="showNavigationmenuBasicCode = !showNavigationmenuBasicCode"
          ...
          >
            ...
            <svg
              :class="showNavigationmenuBasicCode ? 'rotate-180' : ''"
          ...
          <div v-if="showNavigationmenuBasicCode" class="border-t">
            <!-- Tabs (gray title bar) -->
            <div class="flex items-center justify-between bg-muted">
              <div class="flex">
                <button
                  @click="activeNavigationmenuBasicTab = 'auto'"
                  :class="activeNavigationmenuBasicTab === 'auto' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Auto
                </button>
                <button
                  @click="activeNavigationmenuBasicTab = 'vue'"
                  :class="activeNavigationmenuBasicTab === 'vue' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Vue
                </button>
              ...
              <button
                @click="copyCode(activeNavigationmenuBasicTab === 'auto' ? navigationmenuBasicAutoCode : navigationmenuBasicVueCode, 'navigationmenu-basic')"
              ...
              >
                <svg v-if="copiedCode !== 'navigationmenu-basic'" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>
                {{ copiedCode === 'navigationmenu-basic' ? 'Copied!' : 'Copy' }}
              </button>
            </div>
            <!-- Code content with syntax highlighting -->
            <pre class="overflow-x-auto p-4 text-sm bg-[hsl(var(--code-bg))] text-[hsl(var(--code-fg))]"><code :class="'block font-mono !p-0 language-' + (activeNavigationmenuBasicTab === 'auto' ? 'auto' : 'html')">{{ activeNavigationmenuBasicTab === 'auto' ? navigationmenuBasicAutoCode : navigationmenuBasicVueCode }}</code></pre>
          </div>
        </div>
      </div>
