<!-- CalendarPage component - Auto-generated from Auto language -->
...
const showCalendarBasicCode = ref<boolean>(true)
const activeCalendarBasicTab = ref<string>('auto')
...
const calendarBasicAutoCode = `calendar {}
`
const calendarBasicVueCode = `<Calendar />
`
const codeblock1Code = `npx shadcn-vue@latest add calendar`
watch(activeCalendarBasicTab, () => {
...
<template>
  <div class="flex flex-col">
    <div class="flex flex-col gap-4">
      <h1 class="text-4xl font-bold tracking-tight">Calendar</h1>
      A date field component that allows users to select a date or date range.
      ...
      <!-- Merged CalendarBasic Component -->
      <div class="rounded-lg border overflow-hidden">
        ...
        <div class="border-t">
          <button
            @click="showCalendarBasicCode = !showCalendarBasicCode"
          ...
          >
            ...
            <svg
              :class="showCalendarBasicCode ? 'rotate-180' : ''"
          ...
          <div v-if="showCalendarBasicCode" class="border-t">
            <!-- Tabs (gray title bar) -->
            <div class="flex items-center justify-between bg-muted">
              <div class="flex">
                <button
                  @click="activeCalendarBasicTab = 'auto'"
                  :class="activeCalendarBasicTab === 'auto' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Auto
                </button>
                <button
                  @click="activeCalendarBasicTab = 'vue'"
                  :class="activeCalendarBasicTab === 'vue' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Vue
                </button>
              ...
              <button
                @click="copyCode(activeCalendarBasicTab === 'auto' ? calendarBasicAutoCode : calendarBasicVueCode, 'calendar-basic')"
              ...
              >
                <svg v-if="copiedCode !== 'calendar-basic'" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>
                {{ copiedCode === 'calendar-basic' ? 'Copied!' : 'Copy' }}
              </button>
            </div>
            <!-- Code content with syntax highlighting -->
            <pre class="overflow-x-auto p-4 text-sm bg-[hsl(var(--code-bg))] text-[hsl(var(--code-fg))]"><code :class="'block font-mono !p-0 language-' + (activeCalendarBasicTab === 'auto' ? 'auto' : 'html')">{{ activeCalendarBasicTab === 'auto' ? calendarBasicAutoCode : calendarBasicVueCode }}</code></pre>
          </div>
        </div>
      </div>
      ...
      <Table>
        ...
        <TableBody>
          ...
          <TableRow>
            ...
            <TableCell>-</TableCell>
            <TableCell>Locale for formatting</TableCell>
          </TableRow>
          <TableRow>
            ...
            <TableCell>Disables the calendar</TableCell>
          </TableRow>
          <TableRow>
            <TableCell>mode</TableCell>
            <TableCell>string</TableCell>
            <TableCell>"single"</TableCell>
            <TableCell>"single", "range"</TableCell>
