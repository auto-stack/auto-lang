<!-- DropdownmenuPage component - Auto-generated from Auto language -->
...
const showDropdownmenuBasicCode = ref<boolean>(true)
const activeDropdownmenuBasicTab = ref<string>('auto')
...
const dropdownmenuBasicAutoCode = `row (gap: "4") {
    dropdown-menu {}
}
`
const dropdownmenuBasicVueCode = `<div class="flex flex-row gap-4">
  <DropdownMenu />
</div>
`
const codeblock1Code = `npx shadcn-vue@latest add dropdown-menu`
watch(activeDropdownmenuBasicTab, () => {
...
<template>
  <div class="flex flex-col">
    <div class="flex flex-col gap-4">
      ...
      A menu that drops down from a trigger element.
      ...
      <!-- Merged DropdownmenuBasic Component -->
      <div class="rounded-lg border overflow-hidden">
        ...
        <div class="border-t">
          <button
            @click="showDropdownmenuBasicCode = !showDropdownmenuBasicCode"
          ...
          >
            ...
            <svg
              :class="showDropdownmenuBasicCode ? 'rotate-180' : ''"
          ...
          <div v-if="showDropdownmenuBasicCode" class="border-t">
            <!-- Tabs (gray title bar) -->
            <div class="flex items-center justify-between bg-muted">
              <div class="flex">
                <button
                  @click="activeDropdownmenuBasicTab = 'auto'"
                  :class="activeDropdownmenuBasicTab === 'auto' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Auto
                </button>
                <button
                  @click="activeDropdownmenuBasicTab = 'vue'"
                  :class="activeDropdownmenuBasicTab === 'vue' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Vue
                </button>
              ...
              <button
                @click="copyCode(activeDropdownmenuBasicTab === 'auto' ? dropdownmenuBasicAutoCode : dropdownmenuBasicVueCode, 'dropdownmenu-basic')"
              ...
              >
                <svg v-if="copiedCode !== 'dropdownmenu-basic'" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>
                {{ copiedCode === 'dropdownmenu-basic' ? 'Copied!' : 'Copy' }}
              </button>
            </div>
            <!-- Code content with syntax highlighting -->
            <pre class="overflow-x-auto p-4 text-sm bg-[hsl(var(--code-bg))] text-[hsl(var(--code-fg))]"><code :class="'block font-mono !p-0 language-' + (activeDropdownmenuBasicTab === 'auto' ? 'auto' : 'html')">{{ activeDropdownmenuBasicTab === 'auto' ? dropdownmenuBasicAutoCode : dropdownmenuBasicVueCode }}</code></pre>
          </div>
        </div>
      </div>
      ...
      <Table>
        ...
        <TableBody>
          ...
          <TableRow>
            ...
            <TableCell>-</TableCell>
            <TableCell>-</TableCell>
            <TableCell>Keyboard shortcut hint</TableCell>
          </TableRow>
        </TableBody>
      </Table>
    </div>
