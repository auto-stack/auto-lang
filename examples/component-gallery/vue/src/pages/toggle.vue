<!-- TogglePage component - Auto-generated from Auto language -->
...
const showToggleBasicCode = ref<boolean>(true)
const activeToggleBasicTab = ref<string>('auto')
...
const toggleBasicAutoCode = `row (gap: "4") {
...
`
const toggleBasicVueCode = `<div class="flex flex-row gap-4">
  <Switch>
    B
  </Switch>
  <Switch>
...
`
const codeblock1Code = `npx shadcn-vue@latest add toggle`
watch(activeToggleBasicTab, () => {
...
<template>
  <div class="flex flex-col">
    <div class="flex flex-col gap-4">
      <h1 class="text-4xl font-bold tracking-tight">Toggle</h1>
      A two-state button that can be toggled on or off, useful for formatting controls.
      ...
      <div class="rounded-lg border overflow-hidden">
        ...
        <div class="border-t">
          <button
            @click="showToggleBasicCode = !showToggleBasicCode"
          ...
          >
            ...
            <svg
              :class="showToggleBasicCode ? 'rotate-180' : ''"
          ...
          <div v-if="showToggleBasicCode" class="border-t">
            <!-- Tabs (gray title bar) -->
            <div class="flex items-center justify-between bg-muted">
              <div class="flex">
                <button
                  @click="activeToggleBasicTab = 'auto'"
                  :class="activeToggleBasicTab === 'auto' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Auto
                </button>
                <button
                  @click="activeToggleBasicTab = 'vue'"
                  :class="activeToggleBasicTab === 'vue' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Vue
                </button>
              ...
              <button
                @click="copyCode(activeToggleBasicTab === 'auto' ? toggleBasicAutoCode : toggleBasicVueCode, 'toggle-basic')"
              ...
              >
                <svg v-if="copiedCode !== 'toggle-basic'" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>
                {{ copiedCode === 'toggle-basic' ? 'Copied!' : 'Copy' }}
              </button>
            </div>
            <!-- Code content with syntax highlighting -->
            <pre class="overflow-x-auto p-4 text-sm bg-[hsl(var(--code-bg))] text-[hsl(var(--code-fg))]"><code :class="'block font-mono !p-0 language-' + (activeToggleBasicTab === 'auto' ? 'auto' : 'html')">{{ activeToggleBasicTab === 'auto' ? toggleBasicAutoCode : toggleBasicVueCode }}</code></pre>
          </div>
        </div>
      </div>
      ...
      <Table>
        ...
        <TableBody>
          <TableRow>
            ...
            <TableCell>Toggle style variant</TableCell>
          </TableRow>
          <TableRow>
            ...
            <TableCell>Toggle size</TableCell>
          </TableRow>
          <TableRow>
            ...
            <TableCell>Pressed state</TableCell>
          </TableRow>
          <TableRow>
            ...
            <TableCell>false</TableCell>
            <TableCell>true, false</TableCell>
            <TableCell>Disables the toggle</TableCell>
          </TableRow>
        </TableBody>
      </Table>
    </div>
