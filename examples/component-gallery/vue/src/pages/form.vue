<!-- FormPage component - Auto-generated from Auto language -->
...
const showFormBasicCode = ref<boolean>(true)
const activeFormBasicTab = ref<string>('auto')
...
const formBasicAutoCode = `form {
    col (gap: "4") {
        form-item {
            form-label (text: "Username") {}
            input (placeholder: "Enter username") {}
            form-description (text: "This is your public display name.") {}
            form-message {}
        }
        form-item {
...
`
const codeblock1Code = `npx shadcn-vue@latest add form`
watch(activeFormBasicTab, () => {
...
<template>
  <div class="flex flex-col">
    <div class="flex flex-col gap-4">
      <h1 class="text-4xl font-bold tracking-tight">Form</h1>
      A form component with validation support using VeeValidate and Zod schemas.
      ...
      <div class="rounded-lg border overflow-hidden">
        ...
        <div class="border-t">
          <button
            @click="showFormBasicCode = !showFormBasicCode"
          ...
          >
            ...
            <svg
              :class="showFormBasicCode ? 'rotate-180' : ''"
          ...
          <div v-if="showFormBasicCode" class="border-t">
            <!-- Tabs (gray title bar) -->
            <div class="flex items-center justify-between bg-muted">
              <div class="flex">
                <button
                  @click="activeFormBasicTab = 'auto'"
                  :class="activeFormBasicTab === 'auto' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Auto
                </button>
                <button
                  @click="activeFormBasicTab = 'vue'"
                  :class="activeFormBasicTab === 'vue' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Vue
                </button>
              ...
              <button
                @click="copyCode(activeFormBasicTab === 'auto' ? formBasicAutoCode : formBasicVueCode, 'form-basic')"
              ...
              >
                <svg v-if="copiedCode !== 'form-basic'" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>
                {{ copiedCode === 'form-basic' ? 'Copied!' : 'Copy' }}
              </button>
            </div>
            <!-- Code content with syntax highlighting -->
            <pre class="overflow-x-auto p-4 text-sm bg-[hsl(var(--code-bg))] text-[hsl(var(--code-fg))]"><code :class="'block font-mono !p-0 language-' + (activeFormBasicTab === 'auto' ? 'auto' : 'html')">{{ activeFormBasicTab === 'auto' ? formBasicAutoCode : formBasicVueCode }}</code></pre>
          </div>
        </div>
      </div>
      ...
      <Table>
        ...
        <TableBody>
          <TableRow>
            ...
            <TableCell>ZodSchema</TableCell>
            <TableCell>-</TableCell>
            <TableCell>-</TableCell>
            <TableCell>Validation schema</TableCell>
          </TableRow>
          <TableRow>
            <TableCell>initial-values</TableCell>
          ...
          <TableRow>
            <TableCell>on-submit</TableCell>
            <TableCell>function</TableCell>
            <TableCell>-</TableCell>
            <TableCell>-</TableCell>
            <TableCell>Submit handler</TableCell>
          </TableRow>
        </TableBody>
      </Table>
    </div>
