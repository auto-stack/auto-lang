<!-- DrawerPage component - Auto-generated from Auto language -->
...
const showDrawerBasicCode = ref<boolean>(true)
const activeDrawerBasicTab = ref<string>('auto')
...
const drawerBasicAutoCode = `row (gap: "4") {
    drawer {}
    drawer-trigger (text: "Open Drawer") {}
}
`
const drawerBasicVueCode = `<div class="flex flex-row gap-4">
  <Drawer />
  <DrawerTrigger />
</div>
`
const codeblock1Code = `npx shadcn-vue@latest add drawer`
watch(activeDrawerBasicTab, () => {
...
<template>
  <div class="flex flex-col">
    <div class="flex flex-col gap-4">
      ...
      A drawer that slides in from the side of the screen.
      ...
      <div class="rounded-lg border overflow-hidden">
        ...
        <div class="border-t">
          <button
            @click="showDrawerBasicCode = !showDrawerBasicCode"
          ...
          >
            ...
            <svg
              :class="showDrawerBasicCode ? 'rotate-180' : ''"
          ...
          <div v-if="showDrawerBasicCode" class="border-t">
            <!-- Tabs (gray title bar) -->
            <div class="flex items-center justify-between bg-muted">
              <div class="flex">
                <button
                  @click="activeDrawerBasicTab = 'auto'"
                  :class="activeDrawerBasicTab === 'auto' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Auto
                </button>
                <button
                  @click="activeDrawerBasicTab = 'vue'"
                  :class="activeDrawerBasicTab === 'vue' ? 'bg-background text-foreground border-b-2 border-primary -mb-px' : 'text-muted-foreground hover:text-foreground border-b-2 border-transparent'"
                  class="px-4 py-2 text-xs font-medium transition-colors"
                >
                  Vue
                </button>
              ...
              <button
                @click="copyCode(activeDrawerBasicTab === 'auto' ? drawerBasicAutoCode : drawerBasicVueCode, 'drawer-basic')"
              ...
              >
                <svg v-if="copiedCode !== 'drawer-basic'" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>
                {{ copiedCode === 'drawer-basic' ? 'Copied!' : 'Copy' }}
              </button>
            </div>
            <!-- Code content with syntax highlighting -->
            <pre class="overflow-x-auto p-4 text-sm bg-[hsl(var(--code-bg))] text-[hsl(var(--code-fg))]"><code :class="'block font-mono !p-0 language-' + (activeDrawerBasicTab === 'auto' ? 'auto' : 'html')">{{ activeDrawerBasicTab === 'auto' ? drawerBasicAutoCode : drawerBasicVueCode }}</code></pre>
          </div>
        </div>
      </div>
      ...
      <Table>
        ...
        <TableBody>
          <TableRow>
            ...
            <TableCell>Controls drawer visibility</TableCell>
          </TableRow>
          <TableRow>
            ...
            <TableCell>Drawer slide direction</TableCell>
          </TableRow>
          <TableRow>
            ...
            <TableCell>-</TableCell>
            <TableCell>-</TableCell>
            <TableCell>Drawer title</TableCell>
          </TableRow>
        </TableBody>
      </Table>
    </div>
