    ...
    Renderer: core::Renderer,
{
    ...
    fn operate(
        ...
        operation.traverse(&mut |operation| {
            // auto-os patch（PLAN-043 走查，P041-D1 竞态族）：布局树与部件
            // 树错位窗口期 children 可为空——上游 unwrap 会崩掉整个桌面
            // （017-chat 三次复现 + 2026-09-24/25 实机再证）。此处良性跳过
            // 本轮 operation（至多丢一拍操作，不致命）。
    ...
    fn update(
        ...
        // auto-os patch：同 operate 臂——children 空窗期跳过本拍事件。
    ...
    fn mouse_interaction(
        ...
        // auto-os patch：children 空窗期回退默认交互态。
    ...
    fn draw(
        ...
        if let Some(clipped_viewport) = bounds.intersection(viewport) {
            ...
            // auto-os patch：children 空窗期跳过本拍绘制（背景已画）。
